...
int getOptions(int argc, char **argv)
{
    ...
    if (!argCount) {
        if (helpRequested) {
            ...
            mycsout << "Please specify a command!" << '\n'
                << '\n' << usage();
            ret = 1;
            goto just_return;
        }
...
/*#ifdef EC_EOE*/
    ...
    if (matchingCommands.size()) {
        if (matchingCommands.size() == 1) {
            ...
            if (!helpRequested) {
                /*try {*/
                    ...
                    cmd->setForce(force);
                    retval = cmd->execute(commandArgs);
                    ...
                    {
						mycsout << g_err.str() << '\n' << '\n';
						mycsout << cmd->helpString(binaryBaseName);
						retval = 1;
					}
					else if(retval < 0)
					{						
            ...
            mycsout << "Ambiguous command abbreviation! Matching:" << '\n';
        ...
        mycsout << "Unknown command " << commandName << "!" << '\n'
            << '\n' << usage();
        retval = 1;
    }
