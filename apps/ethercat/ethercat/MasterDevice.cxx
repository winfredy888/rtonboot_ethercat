...
int MasterDevice::open(Permissions perm)
{
    ...
    if (fd == -1) { // not already open
        ...
        deviceName << "/EtherCAT" << index;

        if ((fd = ::open(deviceName.str().c_str(),
                        perm == ReadWrite ? O_RDWR : O_RDONLY)) == -1) {
            stringstream err;
            g_err << "Failed to open master device " << deviceName.str() << ": "
                << strerror(errno);
         ...
         /*mycsout << "open success" << '\n';*/
         		
        retval = getModule(&module_data);        
        if(retval)
        {
			return retval;
		}
		
		/*mycsout << "getModule success" << '\n';*/	
        
        if (module_data.ioctl_version_magic != EC_IOCTL_VERSION_MAGIC) {
            stringstream err;
            
            g_err << "ioctl() version magic is differing: "
...
int MasterDevice::getModule(ec_ioctl_module_t *data)
{
	stringstream str;
		
    if (ioctl(fd, EC_IOCTL_MODULE, (unsigned long)((uintptr_t)data) ) < 0) {
        stringstream err;
        
        g_err.str("");
        g_err << "Failed to get module information: " << strerror(errno);
        /*throw MasterDeviceException(err);*/
...
int MasterDevice::getMaster(ec_ioctl_master_t *data)
{
    ...
    if (ioctl(fd, EC_IOCTL_MASTER, data) < 0) {
        ...
        g_err.str("");
        g_err << "Failed to get master information: " << strerror(errno);
...
int MasterDevice::getConfig(ec_ioctl_config_t *data, unsigned int index)
{
    ...
    if (ioctl(fd, EC_IOCTL_CONFIG, data) < 0) {
        ...
        g_err << "Failed to get slave configuration: " << strerror(errno);
        /*throw MasterDeviceException(err);*/
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_CONFIG_PDO, data) < 0) {
        ...
        g_err << "Failed to get slave config PDO: " << strerror(errno);
...
int MasterDevice::getConfigPdoEntry(
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_CONFIG_PDO_ENTRY, data) < 0) {
        ...
        g_err << "Failed to get slave config PDO entry: " << strerror(errno);
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_CONFIG_SDO, data) < 0) {
        ...
        g_err << "Failed to get slave config SDO: " << strerror(errno);
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_CONFIG_IDN, data) < 0) {
        ...
        g_err << "Failed to get slave config IDN: " << strerror(errno);
...
int MasterDevice::getConfigFlag(
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_CONFIG_FLAG, data) < 0) {
        ...
        g_err << "Failed to get slave config flag: " << strerror(errno);
...
int MasterDevice::getDomain(ec_ioctl_domain_t *data, unsigned int index)
{
    data->index = index;
    ...
    if (ioctl(fd, EC_IOCTL_DOMAIN, data)) {
        ...
        g_err << "Failed to get domain: ";
        if (errno == EINVAL)
            g_err << "Domain " << index << " does not exist!";
        else
            g_err << strerror(errno);
...
int MasterDevice::getData(ec_ioctl_domain_data_t *data,
        unsigned int domainIndex, unsigned int dataSize, unsigned char *mem)
{
    ...
    if (ioctl(fd, EC_IOCTL_DOMAIN_DATA, data) < 0) {
        ...
        g_err << "Failed to get domain data: " << strerror(errno);
...
int MasterDevice::getSlave(ec_ioctl_slave_t *slave, uint16_t slaveIndex)
{
    ...
    if (ioctl(fd, EC_IOCTL_SLAVE, slave)) {
        ...
        g_err << "Failed to get slave: ";
        if (errno == EINVAL)
            g_err << "Slave " << slaveIndex << " does not exist!";
        else
            g_err << strerror(errno);
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_DOMAIN_FMMU, fmmu)) {
        ...
        g_err << "Failed to get domain FMMU: " << strerror(errno);
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_SLAVE_SYNC, sync)) {
        ...
        g_err << "Failed to get sync manager: " << strerror(errno);
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_SLAVE_SYNC_PDO, pdo)) {
        ...
        g_err << "Failed to get PDO: " << strerror(errno);
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_SLAVE_SYNC_PDO_ENTRY, entry)) {
        ...
        g_err << "Failed to get PDO entry: " << strerror(errno);
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_SLAVE_SDO, sdo)) {
        ...
        g_err << "Failed to get SDO: " << strerror(errno);
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_SLAVE_SDO_ENTRY, entry)) {
        ...
        g_err << "Failed to get SDO entry: " << strerror(errno);
        ...
        )
{
    if (ioctl(fd, EC_IOCTL_SLAVE_SII_READ, data) < 0) {
        ...
        g_err << "Failed to read SII: " << strerror(errno);
        /*throw MasterDeviceException(err);*/
        ...
        )
{
    if (ioctl(fd, EC_IOCTL_SLAVE_SII_WRITE, data) < 0) {
        ...
        g_err << "Failed to write SII: " << strerror(errno);
        /*throw MasterDeviceException(err);*/
        ...
        )
{
    if (ioctl(fd, EC_IOCTL_SLAVE_REG_READ, data) < 0) {
        ...
        g_err << "Failed to read register: " << strerror(errno);
        ...
        )
{
    if (ioctl(fd, EC_IOCTL_SLAVE_REG_WRITE, data) < 0) {
        ...
        g_err << "Failed to write register: " << strerror(errno);
        ...
        )
{
    if (ioctl(fd, EC_IOCTL_SLAVE_FOE_READ, data) < 0) {
        ...
        g_err << "Failed to read via FoE: " << strerror(errno);
        ...
        )
{
    if (ioctl(fd, EC_IOCTL_SLAVE_FOE_WRITE, data) < 0) {
        ...
        g_err << "Failed to write via FoE: " << strerror(errno);
...
int MasterDevice::setDebug(unsigned int debugLevel)
{
    if (ioctl(fd, EC_IOCTL_MASTER_DEBUG, debugLevel) < 0) {
        ...
        g_err << "Failed to set debug level: " << strerror(errno);
...
int MasterDevice::rescan()
{
    if (ioctl(fd, EC_IOCTL_MASTER_RESCAN, 0) < 0) {
        ...
        g_err << "Failed to command rescan: " << strerror(errno);
...
int MasterDevice::sdoDownload(ec_ioctl_slave_sdo_download_t *data)
{
    if (ioctl(fd, EC_IOCTL_SLAVE_SDO_DOWNLOAD, data) < 0) {
        ...
        if (errno == EIO && data->abort_code) {
			...
			g_err << "Failed to command sdoDownload abort_code " << data->abort_code;
...
int MasterDevice::sdoUpload(ec_ioctl_slave_sdo_upload_t *data)
{
    if (ioctl(fd, EC_IOCTL_SLAVE_SDO_UPLOAD, data) < 0) {
        ...
        if (errno == EIO && data->abort_code) {
            ...
            g_err << "Failed to command sdoUpload abort_code " << data->abort_code;
            
        } else {
            g_err << "Failed to upload SDO: " << strerror(errno);
        ...
        )
{
    ...
    if (ioctl(fd, EC_IOCTL_SLAVE_STATE, &data)) {
        ...
        g_err << "Failed to request slave state: ";
        
        if (errno == EINVAL)
            g_err << "Slave " << slavePosition << " does not exist!";
        else
            g_err << strerror(errno);
...
int MasterDevice::readSoe(ec_ioctl_slave_soe_read_t *data)
{
    if (ioctl(fd, EC_IOCTL_SLAVE_SOE_READ, data) < 0) {
        ...
        if (errno == EIO && data->error_code) {
			...
			g_err << "Failed to command readSoe abort_code " << data->error_code;
			
            /*throw MasterDeviceSoeException(data->error_code);*/
        } else {
            stringstream err;
            
            g_err << "Failed to read IDN: " << strerror(errno);
...
int MasterDevice::writeSoe(ec_ioctl_slave_soe_write_t *data)
{
    if (ioctl(fd, EC_IOCTL_SLAVE_SOE_WRITE, data) < 0) {
        ...
        if (errno == EIO && data->error_code) {
			...
			g_err << "Failed to command writeSoe abort_code " << data->error_code;
            ...
            g_err << "Failed to write IDN: " << strerror(errno);
...
int MasterDevice::setIpParam(ec_ioctl_eoe_ip_t *data)
{
    if (ioctl(fd, EC_IOCTL_SLAVE_EOE_IP_PARAM, data) < 0) {
        if (errno == EIO && data->result) {
			
			g_err << "Failed to setIpParam result " << data->result;
			
            /*throw MasterDeviceEoeException(data->result);*/
        } else {
            stringstream err;
            
            g_err << "Failed to set IP parameters: " << strerror(errno);
