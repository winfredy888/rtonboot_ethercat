...
int CommandSoeRead::execute(const StringVector &args)
{
    SlaveList slaves;
    stringstream err;
    ...
    if (args.size() == 1) {
        ...
        g_err << "'" << getName() << "' takes eiter 1 or 2 arguments!";
    ...
    /*try {*/
        retval = outputData(cout, dataType, ioctl.data, ioctl.data_size);
        if(retval)
        {
			delete [] ioctl.data;
