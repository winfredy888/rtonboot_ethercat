...
int CommandSdos::execute(const StringVector &args)
{
    ...
    for (mi = masterIndices.begin();
        ...
        for (si = slaves.begin(); si != slaves.end(); si++) {
            retval = listSlaveSdos(m, *si, showHeader);
            if(retval)
				return retval;
        }
    }
        ...
        )
{
    ...
    for (i = 0; i < slave.sdo_count; i++) {
        retval = m.getSdo(&sdo, slave.position, i);
        if(retval)
			return retval;
        ...
        for (j = 0; j <= sdo.max_subindex; j++) {
            /*try {*/
                retval = m.getSdoEntry(&entry, slave.position, -i, j);
                if(retval)
					continue;
            /*}*/
            ...
            if ((d = findDataType(entry.data_type))) {
                ...
                str1 << "type " << setw(4) << entry.data_type;
            }

            str1 << ", " << dec << entry.bit_length << " bit, \""
