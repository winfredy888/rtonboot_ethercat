  ...
  if ((lsflags & (LSFLAGS_SIZE | LSFLAGS_LONG | LSFLAGS_UID_GID)) != 0)
    {
      ...
      if ((lsflags & LSFLAGS_SIZE) != 0)
        {
          if (lsflags & LSFLAGS_HUMANREADBLE && buf.st_size >= MYKB)
            {
              if (buf.st_size >= MYGB)
                {
                  nsh_output(vtbl, "%11.1fG", (float)buf.st_size / MYGB);
                }
              else if (buf.st_size >= MYMB)
                {
                  nsh_output(vtbl, "%11.1fM", (float)buf.st_size / MYMB);
                }
              else
                {
                  nsh_output(vtbl, "%11.1fK", (float)buf.st_size / MYKB);
                }
            }
          else
            {
