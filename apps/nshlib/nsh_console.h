...
/*#  define nsh_error(v, ...)    linux_printf(##__VA_ARGS__)
#  define nsh_output(v, ...)   linux_printf(##__VA_ARGS__)
#  define nsh_none(v, ...)     \
     do { if (0) linux_printf(##__VA_ARGS__); } while (0)*/
     
extern int nsh_output(FAR struct nsh_vtbl_s *vtbl, const char *fmt, ...);

extern int nsh_error(FAR struct nsh_vtbl_s *vtbl, const char *fmt, ...);     
