...
# 2. Derive the apps path and get its absolute path
APPS_REL_PATH="../apps"
APPS_DIR="$(cd "$CROSS_COMMON_DIR/$APPS_REL_PATH" && pwd)"

# 3. Define the link description file path (assuming it's in the same directory as the script)
LINKS_FILE="$CROSS_COMMON_DIR/apps_links.txt"
...
echo "Apps root directory: $APPS_DIR"
echo "Processing symbolic links for apps..."
...
while IFS= read -r line || [ -n "$line" ]; do
    ...
    # Both source and target paths are based on the Apps root directory
    src_full_path="$APPS_DIR/$src_rel_path"
    tgt_full_path="$APPS_DIR/$tgt_rel_path"
...
echo "All apps symbolic links processed successfully."
