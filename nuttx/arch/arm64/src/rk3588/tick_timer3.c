...
extern void maincore_arm64_arch_timer_init(void); 

extern void ecmain_timer_irq_init(void);

extern void ecslave_timer_irq_init(void);

#if (ENABLE_JIHUOMA > 0)

extern uint32_t do_soemthing(void);

extern void make_soemthing(uint32_t input);

extern uint8_t g_soemThing[];

uint8_t * ptr_signed = (uint8_t *)( ((unsigned long long)CONFIG_RAM_START) + 0x1e0);

static void make_done(uint8_t ch)
{
	uint64_t temp;
	char * ptr;
...
static struct oneshot_lowerhalf_s *arm64_oneshot_initialize(void)
{
  ...
  /*rtos_printf("TICK_PER_SEC %d, cycle_per_tick %d \n", TICK_PER_SEC, priv->cycle_per_tick);*/
  
  #if (ENABLE_JIHUOMA > 0)
  
	value = do_soemthing();
  
	make_soemthing(value);
  
	is_ok = 1;
	for(int i = 0; i < 32; ++i)
	{
	  if( (g_soemThing[i]) != (ptr_signed[i]) )
	  {
		  is_ok = 0;
  ...
  ecmain_timer_irq_init();
  
  #if !defined(CONFIG_SMP)
  
	ecslave_timer_irq_init();
  ...
  maincore_arm64_arch_timer_init();
...
extern void slavecore_arm64_arch_timer_init(void);
...
//#ifdef CONFIG_SCHED_TICKLESS
  ...
  arm64_arch_timer_enable(true);
	...
	slavecore_arm64_arch_timer_init();
