...
#if defined (CONFIG_ARCH_CHIP_RK3588)

#define RTONBOOT_COMMU_IPI GIC_IRQ_SGI7

extern int rtonboot_commu_handler(int irq, void *context, void *arg);
...
static int real_up_cpu_index(void)
{
	return CONFIG_RTOS_CPUID;	
}

static int real_cpu_index_upon_num(int cpu_num)
{
	if(cpu_num == 0)
		return CONFIG_RTOS_CPUID;
  ...
  if (GIC_IS_SPI(intid))
    {
      ...
      arm64_gic_write_irouter(real_up_cpu_index(), intid);
    }
}
...
static void arm64_write_sgi_val(unsigned int sgi_id, uint64_t sgi_val)
{
  uint32_t regval;
...
int arm64_gic_raise_sgi(unsigned int sgi_id, uint16_t target_list)
{
  ...
  for(i = 0; i < 16; ++i)
  {
	  ...
	  if(target_list & mask)
	  {
		  ...
		  /*linux_printf("Winfred Young write sgi val is %llx\n", val);*/
		  
		  arm64_write_sgi_val(sgi_id, val);
	  }	  
  }	  
...
int arm64_gic_raise_rtonboot_sgi(void)
{
  /*uint64_t pre_cluster_id = UINT64_MAX;
  uint64_t curr_cluster_id;
  uint64_t curr_mpidr;
  ...
  uint16_t cpu = 0;
  uint16_t i;
  unsigned int sgi_id = RTONBOOT_COMMU_IPI;
  uint16_t target_list = 1;
...
#  endif
  ...
  rtos_printf("Winfre Young install rtonboot_commu_handler\n");

  DEBUGVERIFY(irq_attach(RTONBOOT_COMMU_IPI, rtonboot_commu_handler, NULL));
  ...
  if (GIC_IS_SPI(irq))
    {
      ...
      uint64_t mpid = real_cpu_index_upon_num(ffs(cpuset) - 1);
...
static void arm64_gic_init(void)
{
  ...
  gic_rdists[cpu] = CONFIG_GICR_BASE +
                    real_up_cpu_index() * CONFIG_GICR_OFFSET;
...
#  endif
  ...
  rtos_printf("Winfred Young enable RTONBOOT_COMMU_IPI\n");

  up_enable_irq(RTONBOOT_COMMU_IPI);
