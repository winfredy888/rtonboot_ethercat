...
#define PERF_TIMER_BASE      0xFE5F0080 
...
uint32_t g_perf_timer_base = PERF_TIMER_BASE;
...
uint64_t get_perf_timer_count(void)
{
	uint32_t cur_count0, cur_count1;
	uint64_t tmp;
	
	cur_count0 = getreg32( g_perf_timer_base + TIMER_CURRENT_VALUE0 );
	
	cur_count1 = getreg32( g_perf_timer_base + TIMER_CURRENT_VALUE1 );
...
static void performance_start(FAR struct performance_time_s *result)
{
  ...
  putreg32( 0, g_perf_timer_base + TIMER_CTRL);
  
  load_count0 = 0xffffffff;
  load_count1 = 0xffffffff;
  putreg32(load_count0, g_perf_timer_base + TIMER_LOAD_COUNT0);
  putreg32(load_count1, g_perf_timer_base + TIMER_LOAD_COUNT1);
  
  putreg32(TIMER_CLR_INT, g_perf_timer_base + TIMER_INTSTATUS);
  putreg32(TIMER_EN, g_perf_timer_base + TIMER_CTRL);
  
  check:
  
  result->start = get_perf_timer_count();
...
static void performance_end(FAR struct performance_time_s *result)
{
  ...
  result->end = get_perf_timer_count();
...
static size_t performance_gettime(FAR struct performance_time_s *result)
{
  ...
  temp = ((unsigned long long)(result->start - result->end) ) * 41;
...
uint64_t ec_perf_calc_cnt(uint64_t cur_cnt, uint64_t ns)
{
  unsigned long long temp;

  temp = ((unsigned long long)(ns)) / 41;
...
uint64_t ec_perf_get_delay(uint64_t old_cnt, uint64_t cur_cnt)
{
  unsigned long long temp;
...
uint64_t ec_perf_cur_ns(void)
{
  ...
  old_cnt = 0xffffffffffffffffULL;
  
  cur_cnt = get_perf_timer_count();
  ...
  cur_cnt = my_arm64_arch_timer_count();
...
uint64_t ec_perf_cur_ns_cur_cnt(uint64_t * p_cur_cnt)
{
  unsigned long long temp;
  uint64_t old_cnt;
  uint64_t cur_cnt;
...
uint64_t ec_perf_calc_new_cnt(uint64_t ns)
{
  uint64_t new_cnt;
  uint64_t cnt;
  uint64_t cur_cnt;
