...
void arm64_chip_boot(void)
{
  ...
  extern int /*__init*/ stmmac_init(int netport_idx, struct plat_stmmacenet_data *plat_dat, struct stmmac_resources *res,
  ...
  stmmac_init(0, NULL, NULL, 0, 0, 0, 0, 0, 0, 0, 0, 0);
...
void print_no_msg(int is_broadcast, int is_kernel, uint32_t pid)
{
	if(is_broadcast)
	{
		linux_printf("There are not any broadcast msg \n");
	}
	else if(is_kernel)
	{
		linux_printf("There are not any kernel msg \n");
	}
	else
	{
		linux_printf("There are not any msg for pid %d\n", pid);
	}
}
...
void print_single_msg(struct rtonboot_msg_header * p_msg_header)
{
	...
	linux_printf("msg id: %d type: %s max_content_size: %d pid: %d offset_content: %d is_kernel: %d\n", 
...
void scan_list_find_this_pid_entry_and_list(int spid)
{
	...
	if(list_empty(p_root_list_head))
	{
		linux_printf("There are not any msg\n");
...
extern void test_linux_vprintf(const char *fmt, ...);
