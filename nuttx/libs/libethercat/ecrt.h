 ...
 *
 * - Added the lib_ecrt_master_scan_progress() method, the
 *   ec_master_scan_progress_t structure and the EC_HAVE_SCAN_PROGRESS
 *   definition to check for its existence.
 * - Added the EoE configuration methods lib_ecrt_slave_config_eoe_mac_address(),
 *   lib_ecrt_slave_config_eoe_ip_address(), lib_ecrt_slave_config_eoe_subnet_mask(),
 *   lib_ecrt_slave_config_eoe_default_gateway(),
 *   lib_ecrt_slave_config_eoe_dns_address(),
 *   lib_ecrt_slave_config_eoe_hostname() and the EC_HAVE_SET_IP
 *   definition to check for its existence.
 * - Added lib_ecrt_slave_config_state_timeout() to set the application-layer
 *   state change timeout and EC_HAVE_STATE_TIMEOUT to check for its
 *   existence.
 *
 * Changes since version 1.5.2:
 *
 * - Added the lib_ecrt_slave_config_flag() method and the EC_HAVE_FLAGS
 *   definition to check for its existence.
 * - Added SoE IDN requests, including the datatype ec_soe_request_t and the
 *   methods lib_ecrt_slave_config_create_soe_request(),
 *   lib_ecrt_soe_request_object(), lib_ecrt_soe_request_timeout(),
 *   lib_ecrt_soe_request_data(), lib_ecrt_soe_request_data_size(),
 *   lib_ecrt_soe_request_state(), lib_ecrt_soe_request_write() and
 *   lib_ecrt_soe_request_read(). Use the EC_HAVE_SOE_REQUESTS to check, if the
 *   functionality is available.
 *
 * Changes in version 1.5.2:
 *
 * - Added redundancy_active flag to ec_domain_state_t.
 * - Added lib_ecrt_master_link_state() method and ec_master_link_state_t to query
 *   the state of a redundant link.
 * - Added the EC_HAVE_REDUNDANCY define, to check, if the interface contains
 *   redundancy features.
 * - Added lib_ecrt_sdo_request_index() to change SDO index and subindex after
 *   request creation.
 * - Added interface for retrieving CoE emergency messages, i. e.
 *   lib_ecrt_slave_config_emerg_size(), lib_ecrt_slave_config_emerg_pop(),
 *   lib_ecrt_slave_config_emerg_clear(), lib_ecrt_slave_config_emerg_overruns() and
 ...
 *   ec_reg_request_t and methods lib_ecrt_slave_config_create_reg_request(),
 *   lib_ecrt_reg_request_data(), lib_ecrt_reg_request_state(),
 *   lib_ecrt_reg_request_write(), lib_ecrt_reg_request_read() and the feature flag
 *   EC_HAVE_REG_ACCESS.
 * - Added method to select the reference clock,
 *   lib_ecrt_master_select_reference_clock() and the feature flag
 *   EC_HAVE_SELECT_REF_CLOCK to check, if the method is available.
 * - Added method to get the reference clock time,
 *   lib_ecrt_master_reference_clock_time() and the feature flag
 *   EC_HAVE_REF_CLOCK_TIME to have the possibility to synchronize the master
 *   clock to the reference clock.
 * - Changed the data types of the shift times in lib_ecrt_slave_config_dc() to
 *   int32_t to correctly display negative shift times.
 * - Added lib_ecrt_slave_config_reg_pdo_entry_pos() and the feature flag
 ...
 *   lib_ecrt_slave_config_dc() to configure a slave for cyclic operation, and
 *   lib_ecrt_master_application_time(), lib_ecrt_master_sync_reference_clock() and
 *   lib_ecrt_master_sync_slave_clocks() for offset and drift compensation. The
 *   EC_TIMEVAL2NANO() macro can be used for epoch time conversion, while the
 *   lib_ecrt_master_sync_monitor_queue() and lib_ecrt_master_sync_monitor_process()
 *   methods can be used to monitor the synchrony.
 * - Improved the callback mechanism. lib_ecrt_master_callbacks() now takes two
 *   callback functions for sending and receiving datagrams.
 *   lib_ecrt_master_send_ext() is used to execute the sending of non-application
 *   datagrams.
 * - Added watchdog configuration (method lib_ecrt_slave_config_watchdog(),
 *   #ec_watchdog_mode_t, \a watchdog_mode parameter in ec_sync_info_t and
 *   lib_ecrt_slave_config_sync_manager()).
 * - Added lib_ecrt_slave_config_complete_sdo() method to download an SDO during
 *   configuration via CompleteAccess.
 * - Added lib_ecrt_master_deactivate() to remove the master configuration.
 * - Added lib_ecrt_open_master() and lib_ecrt_master_reserve() separation for
 *   userspace.
 * - Added master information interface (methods lib_ecrt_master(),
 *   lib_ecrt_master_get_slave(), lib_ecrt_master_get_sync_manager(),
 *   lib_ecrt_master_get_pdo() and lib_ecrt_master_get_pdo_entry()) to get information
 *   about the currently connected slaves and the PDO entries provided.
 * - Added lib_ecrt_master_sdo_download(), lib_ecrt_master_sdo_download_complete() and
 *   lib_ecrt_master_sdo_upload() methods to let an application transfer SDOs
 *   before activating the master.
 * - Changed the meaning of the negative return values of
 *   lib_ecrt_slave_config_reg_pdo_entry() and lib_ecrt_slave_config_sdo*().
 * - Implemented the Vendor-specific over EtherCAT mailbox protocol. See
 *   lib_ecrt_slave_config_create_voe_handler().
 * - Renamed ec_sdo_request_state_t to #ec_request_state_t, because it is also
 *   used by VoE handlers.
 * - Removed 'const' from argument of lib_ecrt_sdo_request_state(), because the
 *   userspace library has to modify object internals.
 * - Added 64-bit data access macros.
 * - Added lib_ecrt_slave_config_idn() method for storing SoE IDN configurations,
 *   and lib_ecrt_master_read_idn() and lib_ecrt_master_write_idn() to read/write IDNs
 *   ad-hoc via the user-space library.
 * - Added lib_ecrt_master_reset() to initiate retrying to configure slaves.
 *
 * @{
 */
...
#ifndef __lib_ecrt_H__
#define __lib_ecrt_H__
 ...
 */
#define lib_ecrt_VER_MAJOR 1

/** EtherCAT realtime interface minor version number.
 */
#define lib_ecrt_VER_MINOR 6

/** EtherCAT realtime interface version word generator.
 */
#define lib_ecrt_VERSION(a, b) (((a) << 8) + (b))

/** EtherCAT realtime interface version word.
 */
#define lib_ecrt_VERSION_MAGIC lib_ecrt_VERSION(lib_ecrt_VER_MAJOR, lib_ecrt_VER_MINOR)
 ...
 * lib_ecrt_master_link_state() method are available.
 */
#define EC_HAVE_REDUNDANCY

/** Defined, if the CoE emergency ring feature is available.
 *
 * I. e. if the lib_ecrt_slave_config_emerg_*() methods are available.
 */
#define EC_HAVE_EMERGENCY

/** Defined, if the register access interface is available.
 *
 * I. e. if the methods lib_ecrt_slave_config_create_reg_request(),
 * lib_ecrt_reg_request_data(), lib_ecrt_reg_request_state(), lib_ecrt_reg_request_write()
 * and lib_ecrt_reg_request_read() are available.
 */
#define EC_HAVE_REG_ACCESS

/** Defined if the method lib_ecrt_master_select_reference_clock() is available.
 */
#define EC_HAVE_SELECT_REF_CLOCK

/** Defined if the method lib_ecrt_master_reference_clock_time() is available.
 */
#define EC_HAVE_REF_CLOCK_TIME

/** Defined if the method lib_ecrt_slave_config_reg_pdo_entry_pos() is available.
 */
#define EC_HAVE_REG_BY_POS

/** Defined if the method lib_ecrt_master_sync_reference_clock_to() is available.
 */
#define EC_HAVE_SYNC_TO

/** Defined if the method lib_ecrt_slave_config_flag() is available.
 */
#define EC_HAVE_FLAGS

/** Defined if the methods lib_ecrt_slave_config_create_soe_request(),
 * lib_ecrt_soe_request_object(), lib_ecrt_soe_request_timeout(),
 * lib_ecrt_soe_request_data(), lib_ecrt_soe_request_data_size(),
 * lib_ecrt_soe_request_state(), lib_ecrt_soe_request_write() and
 * lib_ecrt_soe_request_read() and the datatype ec_soe_request_t are available.
 */
#define EC_HAVE_SOE_REQUESTS

/** Defined, if the method lib_ecrt_master_scan_progress() and the
 * ec_master_scan_progress_t structure are available.
 */
#define EC_HAVE_SCAN_PROGRESS

/** Defined, if the methods lib_ecrt_slave_config_eoe_mac_address(),
 * lib_ecrt_slave_config_eoe_ip_address(), lib_ecrt_slave_config_eoe_subnet_mask(),
 * lib_ecrt_slave_config_eoe_default_gateway(),
 * lib_ecrt_slave_config_eoe_dns_address(), lib_ecrt_slave_config_eoe_hostname() are
 * available.
 */
#define EC_HAVE_SET_IP

/** Defined, if the method lib_ecrt_slave_config_state_timeout() is available.
 */
...
/** End of list marker.
 *
 * This can be used with lib_ecrt_slave_config_pdos().
 */
#define EC_END ~0U
 ...
 *
 * \see void lib_ecrt_master_application_time()
 *
 ...
 *
 * \see lib_ecrt_slave_config_emerg_pop().
 */
...
/** Master state.
 *
 * This is used for the output parameter of lib_ecrt_master_state().
 *
 * \see lib_ecrt_master_state().
 */
typedef struct {
 ...
 *
 * This is used for the output parameter of lib_ecrt_master_link_state().
 *
 * \see lib_ecrt_master_link_state().
 */
typedef struct {
 ...
 *
 * This is used as an output parameter of lib_ecrt_slave_config_state().
 *
 * \see lib_ecrt_slave_config_state().
 */
typedef struct  {
...
/** Master information.
 *
 * This is used as an output parameter of lib_ecrt_master().
 *
 * \see lib_ecrt_master().
 */
typedef struct {
 ...
 *
 * This is used as an output parameter of lib_ecrt_master_scan_progress().
 *
 * \see lib_ecrt_master_scan_progress().
 */
typedef struct {
...
/** Slave information.
 *
 * This is used as an output parameter of lib_ecrt_master_get_slave().
 *
 * \see lib_ecrt_master_get_slave().
 */
typedef struct {
...
/** Domain state.
 *
 * This is used for the output parameter of lib_ecrt_domain_state().
 */
typedef struct {
 ...
 *
 * \see lib_ecrt_slave_config_pdos().
 */
typedef struct {
 ...
 * lib_ecrt_domain_reg_pdo_entry_list()
 */
typedef struct {
...
/** Request state.
 *
 * This is used as return type for lib_ecrt_sdo_request_state() and
 * lib_ecrt_voe_handler_state().
 */
typedef enum {
 ...
 *
 * \return Value of lib_ecrt_VERSION_MAGIC() at EtherCAT master compile time.
 */
EC_PUBLIC_API unsigned int lib_ecrt_version_magic(void);
 ...
 * for exclusive use.
 *
 * In userspace, this is a convenience function for lib_ecrt_open_master() and
 * lib_ecrt_master_reserve().
 *
 ...
 */
EC_PUBLIC_API ec_master_t *lib_ecrt_request_master(
 ...
 * of masters has to be specified when loading the master module.
 *
 * For convenience, the function lib_ecrt_request_master() can be used.
 *
 * \apiusage{master_idle,blocking}
 *
 * \return Pointer to the opened master, otherwise \a NULL.
 */
EC_PUBLIC_API ec_master_t *lib_ecrt_open_master(
        unsigned int master_index /**< Index of the master to request. */
        );
 ...
 * realtime context.
 *
 * If the master was activated, lib_ecrt_master_deactivate() is called internally.
 *
 * \apiusage{master_any,blocking}
 */
EC_PUBLIC_API void lib_ecrt_release_master(
 ...
 */
EC_PUBLIC_API int lib_ecrt_master_reserve(
        ec_master_t *master /**< EtherCAT master */
        );
 ...
 * lib_ecrt_master_send_ext() method.
 *
 * The task of the receive callback (\a receive_cb) is to decide, if a call to
 * lib_ecrt_master_receive() is allowed and to execute it respectively.
 *
 * \apiusage{master_idle,blocking}
 *
 * \attention This method has to be called before lib_ecrt_master_activate().
 */
void lib_ecrt_master_callbacks(
        ec_master_t *master, /**< EtherCAT master */
 ...
 * before lib_ecrt_master_activate().
 *
 * \apiusage{master_idle,blocking}
 *
 * \return Pointer to the new domain on success, else NULL.
 */
EC_PUBLIC_API ec_domain_t *lib_ecrt_master_create_domain(
        ec_master_t *master /**< EtherCAT master. */
        );
 ...
 */
EC_PUBLIC_API ec_slave_config_t *lib_ecrt_master_slave_config(
        ec_master_t *master, /**< EtherCAT master */
 ...
 */
EC_PUBLIC_API int lib_ecrt_master_select_reference_clock(
        ec_master_t *master, /**< EtherCAT master. */
 ...
 * \return 0 in case of success, else < 0
 */
EC_PUBLIC_API int lib_ecrt_master(
        ec_master_t *master, /**< EtherCAT master */
 ...
 * \return 0 in case of success, else < 0
 */
EC_PUBLIC_API int lib_ecrt_master_scan_progress(
        ec_master_t *master, /**< EtherCAT master */
 ...
 * \return 0 in case of success, else < 0
 */
EC_PUBLIC_API int lib_ecrt_master_get_slave(
        ec_master_t *master, /**< EtherCAT master */
 ...
 * lib_ecrt_master_get_pdo() to get the PDO information.
 *
 * \apiusage{master_any,blocking}
 *
 * \return zero on success, else non-zero
 */
EC_PUBLIC_API int lib_ecrt_master_get_sync_manager(
        ec_master_t *master, /**< EtherCAT master. */
        uint16_t slave_position, /**< Slave position. */
 ...
 * value is left empty. Use lib_ecrt_master_get_pdo_entry() to get the PDO
 * entry information.
 *
 * \apiusage{master_any,blocking}
 *
 ...
 */
EC_PUBLIC_API int lib_ecrt_master_get_pdo(
        ec_master_t *master, /**< EtherCAT master. */
        uint16_t slave_position, /**< Slave position. */
        uint8_t sync_index, /**< Sync manager index. Must be less
                                 than #EC_MAX_SYNC_MANAGERS. */
 ...
 * \retval zero on success, else non-zero
 */
EC_PUBLIC_API int lib_ecrt_master_get_pdo_entry(
        ec_master_t *master, /**< EtherCAT master. */
        uint16_t slave_position, /**< Slave position. */
        uint8_t sync_index, /**< Sync manager index. Must be less
                                 than #EC_MAX_SYNC_MANAGERS. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_master_sdo_download(
        ec_master_t *master, /**< EtherCAT master. */
        uint16_t slave_position, /**< Slave position. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_master_sdo_download_complete(
        ec_master_t *master, /**< EtherCAT master. */
        uint16_t slave_position, /**< Slave position. */
        uint16_t index, /**< Index of the SDO. */
        const uint8_t *data, /**< Data buffer to download. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_master_sdo_upload(
        ec_master_t *master, /**< EtherCAT master. */
        uint16_t slave_position, /**< Slave position. */
        uint16_t index, /**< Index of the SDO. */
        uint8_t subindex, /**< Subindex of the SDO. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_master_write_idn(
        ec_master_t *master, /**< EtherCAT master. */
        uint16_t slave_position, /**< Slave position. */
        uint8_t drive_no, /**< Drive number. */
        uint16_t idn, /**< SoE IDN (see lib_ecrt_slave_config_idn()). */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_master_read_idn(
        ec_master_t *master, /**< EtherCAT master. */
        uint16_t slave_position, /**< Slave position. */
        uint8_t drive_no, /**< Drive number. */
        uint16_t idn, /**< SoE IDN (see lib_ecrt_slave_config_idn()). */
 ...
 * in charge of cyclically calling lib_ecrt_master_send() and
 * lib_ecrt_master_receive() to ensure network communication. Before calling this
 ...
 * \return 0 in case of success, else < 0
 */
EC_PUBLIC_API int lib_ecrt_master_activate(
        ec_master_t *master /**< EtherCAT master. */
        );
 ...
 * lib_ecrt_master_create_domain(), lib_ecrt_master_slave_config(), lib_ecrt_domain_data()
 * lib_ecrt_slave_config_create_sdo_request() and
 * lib_ecrt_slave_config_create_voe_handler() are freed, so pointers to them
 * become invalid.
 *
 ...
 */
EC_PUBLIC_API int lib_ecrt_master_deactivate(
        ec_master_t *master /**< EtherCAT master. */
        );

/** Set interval between calls to lib_ecrt_master_send().
 *
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_master_set_send_interval(
        ec_master_t *master, /**< EtherCAT master. */
 ...
 *
 * Has to be called cyclically by the application after lib_ecrt_master_activate()
 * has returned.
 *
 ...
 */
EC_PUBLIC_API int lib_ecrt_master_send(
        ec_master_t *master /**< EtherCAT master. */
        );
 ...
 * lib_ecrt_master_activate() has returned.
 *
 * \apiusage{master_op,rt_safe}
 *
 * \return Zero on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_master_receive(
        ec_master_t *master /**< EtherCAT master. */
        );
 ...
 * lib_ecrt_master_callbacks() to allow the sending of non-application datagrams.
 *
 * \apiusage{master_op,rt_safe}
 *
 * \return Zero on success, otherwise negative error code.
 ...
 * redundant network topology, use the lib_ecrt_master_link_state() method.
 *
 * \apiusage{master_any,rt_safe}
 *
 * \return Zero on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_master_state(
 ...
 * \return Zero on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_master_link_state(
        const ec_master_t *master, /**< EtherCAT master. */
 ...
 * lib_ecrt_master_sync_reference_clock().
 *
 ...
 * \return Zero on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_master_application_time(
        ec_master_t *master, /**< EtherCAT master. */
        uint64_t app_time /**< Application time. */
        );
 ...
 * by the last call off lib_ecrt_master_application_time().
 *
 * \apiusage{master_op,rt_safe}
 *
 * \return Zero on success, otherwise negative error code.
 ...
 */
EC_PUBLIC_API int lib_ecrt_master_sync_reference_clock(
        ec_master_t *master /**< EtherCAT master. */
        );
 ...
 * sync_time parameter.
 *
 * Has to be called by the application after lib_ecrt_master_activate()
 * has returned.
 *
 * \apiusage{master_op,rt_safe}
 *
 ...
 * \retval -ENXIO No reference clock found.
 */
EC_PUBLIC_API int lib_ecrt_master_sync_reference_clock_to(
        ec_master_t *master, /**< EtherCAT master. */
 ...
 * \retval -ENXIO No reference clock found.
 */
EC_PUBLIC_API int lib_ecrt_master_sync_slave_clocks(
        ec_master_t *master /**< EtherCAT master. */
        );
 ...
 * lib_ecrt_master_sync_slave_clocks() method, that reads the system time of the
 ...
 * activation), when the lib_ecrt_master_sync_slave_clocks() method is called
 * cyclically.
 *
 * \apiusage{master_op,rt_safe}
 *
 ...
 */
EC_PUBLIC_API int lib_ecrt_master_reference_clock_time(
        const ec_master_t *master, /**< EtherCAT master. */
 ...
 * checked with the lib_ecrt_master_sync_monitor_process() method.
 *
 * \apiusage{master_op,rt_safe}
 *
 * \return Zero on success, otherwise a negative error code.
 */
EC_PUBLIC_API int lib_ecrt_master_sync_monitor_queue(
        ec_master_t *master /**< EtherCAT master. */
        );
 ...
 * lib_ecrt_master_sync_monitor_queue(), the result can be queried with this
 * method.
 *
 * \apiusage{master_op,rt_safe}
 *
 ...
 */
EC_PUBLIC_API uint32_t lib_ecrt_master_sync_monitor_process(
        const ec_master_t *master /**< EtherCAT master. */
        );
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_master_reset(
        ec_master_t *master /**< EtherCAT master. */
        );
 ...
 * lib_ecrt_master_activate().
 *
 * \apiusage{master_idle,blocking}
 *
 * \return zero on success, else non-zero
 */
EC_PUBLIC_API int lib_ecrt_slave_config_sync_manager(
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_watchdog(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * \return zero on success, else non-zero
 */
EC_PUBLIC_API int lib_ecrt_slave_config_pdo_assign_add(
        ec_slave_config_t *sc, /**< Slave configuration. */
        uint8_t sync_index, /**< Sync manager index. Must be less
                              than #EC_MAX_SYNC_MANAGERS. */
 ...
 * lib_ecrt_slave_config_pdo_assign_add(), to clear the default assignment of a
 * sync manager.
 *
 * This method has to be called in non-realtime context before
 * lib_ecrt_master_activate().
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_pdo_assign_clear(
        ec_slave_config_t *sc, /**< Slave configuration. */
        uint8_t sync_index /**< Sync manager index. Must be less
                              than #EC_MAX_SYNC_MANAGERS. */
        );
 ...
 * \return zero on success, else non-zero
 */
EC_PUBLIC_API int lib_ecrt_slave_config_pdo_mapping_add(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * lib_ecrt_slave_config_pdo_mapping_add(), to clear the default mapping.
 *
 * This method has to be called in non-realtime context before
 * lib_ecrt_master_activate().
 *
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_pdo_mapping_clear(
        ec_slave_config_t *sc, /**< Slave configuration. */
        uint16_t pdo_index /**< Index of the PDO. */
        );
 ...
 * lib_ecrt_slave_config_sync_manager(), lib_ecrt_slave_config_pdo_assign_clear(),
 * lib_ecrt_slave_config_pdo_assign_add(), lib_ecrt_slave_config_pdo_mapping_clear()
 * and lib_ecrt_slave_config_pdo_mapping_add(), that are better suitable for
 ...
 * };
 *
 * if (lib_ecrt_slave_config_pdos(sc_ana_in, EC_END, el3162_syncs)) {
 *     // handle error
 * }
 * \endcode
 *
 ...
 * };
 *
 * if (lib_ecrt_slave_config_pdos(slave_config_ana_in, 1, syncs)) {
 *     // handle error
 * }
 * \endcode
 *
 ...
 * \return zero on success, else non-zero
 */
EC_PUBLIC_API int lib_ecrt_slave_config_pdos(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * \retval  <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_reg_pdo_entry(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 *
 * Similar to lib_ecrt_slave_config_reg_pdo_entry(), but not using PDO indices but
 ...
 * \retval  <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_reg_pdo_entry_pos(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_dc(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * configuration done by the master. Please use lib_ecrt_slave_config_pdos() and
 * friends instead.
 *
 ...
 * endianness), have a look at lib_ecrt_slave_config_sdo8(),
 * lib_ecrt_slave_config_sdo16() and lib_ecrt_slave_config_sdo32().
 *
 * This method has to be called in non-realtime context before
 * lib_ecrt_master_activate().
 *
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_sdo(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_sdo8(
        ec_slave_config_t *sc, /**< Slave configuration */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_sdo16(
        ec_slave_config_t *sc, /**< Slave configuration */
        uint16_t sdo_index, /**< Index of the SDO to configure. */
        uint8_t sdo_subindex, /**< Subindex of the SDO to configure. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_sdo32(
        ec_slave_config_t *sc, /**< Slave configuration */
        uint16_t sdo_index, /**< Index of the SDO to configure. */
        uint8_t sdo_subindex, /**< Subindex of the SDO to configure. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_complete_sdo(
        ec_slave_config_t *sc, /**< Slave configuration. */
        uint16_t index, /**< Index of the SDO to configure. */
        const uint8_t *data, /**< Pointer to the data. */
        size_t size /**< Size of the \a data. */
 ...
 */
EC_PUBLIC_API int lib_ecrt_slave_config_emerg_size(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 */
EC_PUBLIC_API int lib_ecrt_slave_config_emerg_pop(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 *
 */
EC_PUBLIC_API int lib_ecrt_slave_config_emerg_clear(
        ec_slave_config_t *sc /**< Slave configuration. */
        );
 ...
 * lib_ecrt_slave_config_emerg_clear() to reset the counter.
 *
 * Calling this method makes only sense in realtime context (after master
 * activation).
 *
 ...
 *
 */
EC_PUBLIC_API int lib_ecrt_slave_config_emerg_overruns(
 ...
 */
EC_PUBLIC_API ec_sdo_request_t *lib_ecrt_slave_config_create_sdo_request(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 */
EC_PUBLIC_API ec_soe_request_t *lib_ecrt_slave_config_create_soe_request(
        ec_slave_config_t *sc, /**< Slave configuration. */
        uint8_t drive_no, /**< Drive number. */
 ...
 */
EC_PUBLIC_API ec_voe_handler_t *lib_ecrt_slave_config_create_voe_handler(
        ec_slave_config_t *sc, /**< Slave configuration. */
        size_t size /**< Data size to reserve. */
        );
 ...
 */
EC_PUBLIC_API ec_reg_request_t *lib_ecrt_slave_config_create_reg_request(
        ec_slave_config_t *sc, /**< Slave configuration. */
        size_t size /**< Data size to reserve. */
        );
 ...
 * realtime, lib_ecrt_domain_state() should be used.
 *
 * \apiusage{master_op,rt_safe}
 *
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_state(
        const ec_slave_config_t *sc, /**< Slave configuration */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_idn(
        ec_slave_config_t *sc, /**< Slave configuration. */
        uint8_t drive_no, /**< Drive number. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_flag(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_eoe_mac_address(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 *         my_fprintf(stderr, "Failed to convert IP address.\n");
 *         return -1;
 *     }
 *     if (lib_ecrt_slave_config_eoe_ip_address(sc, addr)) {
 *         my_fprintf(stderr, "Failed to set IP address.\n");
 *         return -1;
 *     }
 * \endcode
 *
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_eoe_ip_address(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 *
 * See lib_ecrt_slave_config_eoe_ip_address() on how to convert string-coded masks
 * to `struct in_addr`.
 *
 * \apiusage{master_idle,blocking}
 *
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_eoe_subnet_mask(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * contains the 32-bit IPv4 address in network byte order (big endian).
 *
 * See lib_ecrt_slave_config_eoe_ip_address() on how to convert string-coded IPv4
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_eoe_default_gateway(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_eoe_dns_address(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_eoe_hostname(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * \retval <0 Error code.
 */
EC_PUBLIC_API int lib_ecrt_slave_config_state_timeout(
        ec_slave_config_t *sc, /**< Slave configuration. */
 ...
 * lib_ecrt_master_activate().
 *
 * \see lib_ecrt_slave_config_reg_pdo_entry()
 *
 ...
 */
EC_PUBLIC_API int lib_ecrt_domain_reg_pdo_entry_list(
        ec_domain_t *domain, /**< Domain. */
 ...
 */
EC_PUBLIC_API size_t lib_ecrt_domain_size(
 ...
 * the master.
 *
 * The size of the allocated memory must be at least lib_ecrt_domain_size(), after
 ...
 * \apiusage{master_idle,blocking}
 */
void lib_ecrt_domain_external_memory(
        ec_domain_t *domain, /**< Domain. */
 ...
 * lib_ecrt_domain_external_memory(), the returned pointer will contain the
 ...
 * lib_ecrt_master_activate() to get the mapped domain process data memory.
 *
 * \apiusage{master_op,rt_safe}
 *
 * \return Pointer to the process data memory.
 */
EC_PUBLIC_API uint8_t *lib_ecrt_domain_data(
        const ec_domain_t *domain /**< Domain. */
        );
 ...
 * statistics, if necessary. This must be called after lib_ecrt_master_receive()
 * is expected to receive the domain datagrams in order to make
 * lib_ecrt_domain_state() return the result of the last process data exchange.
 *
 * \apiusage{master_op,rt_safe}
 *
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_domain_process(
        ec_domain_t *domain /**< Domain. */
        );
 ...
 * next call of lib_ecrt_master_send().
 *
 * \apiusage{master_op,rt_safe}
 *
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_domain_queue(
        ec_domain_t *domain /**< Domain. */
        );
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_domain_state(
        const ec_domain_t *domain, /**< Domain. */
 ...
 * lib_ecrt_sdo_request_state() returns EC_REQUEST_BUSY, this may lead to
 * unexpected results.
 *
 * This method is meant to be called in realtime context (after master
 * activation). To initialize the SDO request, the index and subindex can be
 * set via lib_ecrt_slave_config_create_sdo_request().
 *
 * \apiusage{master_op,rt_safe}
 *
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_sdo_request_index(
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_sdo_request_timeout(
        ec_sdo_request_t *req, /**< SDO request. */
 ...
 *   \code
 *   uint16_t value = EC_READ_U16(lib_ecrt_sdo_request_data(sdo)));
 *   \endcode
 ...
 *   is a parameter of lib_ecrt_slave_config_create_sdo_request().
 *   \code
 *   EC_WRITE_U16(lib_ecrt_sdo_request_data(sdo), 0xFFFF);
 *   \endcode
 *
 ...
 *
 */
EC_PUBLIC_API uint8_t *lib_ecrt_sdo_request_data(
 ...
 *
 */
EC_PUBLIC_API size_t lib_ecrt_sdo_request_data_size(
        const ec_sdo_request_t *req /**< SDO request. */
        );
 ...
 *
 */
EC_PUBLIC_API ec_request_state_t lib_ecrt_sdo_request_state(
#ifdef __KERNEL__
        const
#endif
        ec_sdo_request_t *req /**< SDO request. */
 ...
 *
 * \attention This method may not be called while lib_ecrt_sdo_request_state()
 ...
 * \retval -ENOBUFS Reserved memory in lib_ecrt_slave_config_create_sdo_request()
 *              too small.
 */
EC_PUBLIC_API int lib_ecrt_sdo_request_write(
        ec_sdo_request_t *req /**< SDO request. */
        );
 ...
 * lib_ecrt_sdo_request_data() must be considered as invalid while
 * lib_ecrt_sdo_request_state() returns EC_REQUEST_BUSY.
 *
 * This method is meant to be called in realtime context (after master
 * activation).
 *
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_sdo_request_read(
        ec_sdo_request_t *req /**< SDO request. */
        );
 ...
 * lib_ecrt_soe_request_state() returns EC_REQUEST_BUSY, this may lead to
 * unexpected results.
 *
 * This method is meant to be called in realtime context (after master
 * activation). To initialize the SoE request, the drive_no and IDN can be
 * set via lib_ecrt_slave_config_create_soe_request().
 *
 * \apiusage{master_op,rt_safe}
 *
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_soe_request_idn(
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_soe_request_timeout(
 ...
 *   using the EC_READ_*() macros as usual. Example:
 *   \code
 *   uint16_t value = EC_READ_U16(lib_ecrt_soe_request_data(idn_req)));
 *   \endcode
 * - If a write operation shall be triggered, the data have to be written to
 *   the internal memory. Use the EC_WRITE_*() macros, if you are writing
 *   integer data. Be sure, that the data fit into the memory. The memory size
 *   is a parameter of lib_ecrt_slave_config_create_soe_request().
 *   \code
 *   EC_WRITE_U16(lib_ecrt_soe_request_data(idn_req), 0xFFFF);
 *   \endcode
 *
 ...
 *
 */
EC_PUBLIC_API uint8_t *lib_ecrt_soe_request_data(
 ...
 */
EC_PUBLIC_API size_t lib_ecrt_soe_request_data_size(
        const ec_soe_request_t *req /**< SoE request. */
        );
 ...
 * \apiusage{master_op,rt_safe}
 */
EC_PUBLIC_API ec_request_state_t lib_ecrt_soe_request_state(
#ifdef __KERNEL__
        const
#endif
        ec_soe_request_t *req /**< SoE request. */
 ...
 *
 * \attention This method may not be called while lib_ecrt_soe_request_state()
 * returns EC_REQUEST_BUSY.
 *
 * This method is meant to be called in realtime context (after master
 * activation).
 ...
 * \return 0 on success, otherwise negative error code.
 * \retval -EINVAL Invalid input data, e.g. data size == 0.
 * \retval -ENOBUFS Reserved memory in lib_ecrt_slave_config_create_soe_request()
 *              too small.
 */
EC_PUBLIC_API int lib_ecrt_soe_request_write(
        ec_soe_request_t *req /**< SoE request. */
        );
 ...
 *
 * \attention After calling this function, the return value of
 * lib_ecrt_soe_request_data() must be considered as invalid while
 * lib_ecrt_soe_request_state() returns EC_REQUEST_BUSY.
 *
 * This method is meant to be called in realtime context (after master
 * activation).
 *
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_soe_request_read(
        ec_soe_request_t *req /**< SoE request. */
        );
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_voe_handler_send_header(
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_voe_handler_received_header(
 ...
 * lib_ecrt_voe_handler_send_header()).
 *
 ...
 *   lib_ecrt_voe_handler_data_size().
 ...
 *   memory size is a parameter of lib_ecrt_slave_config_create_voe_handler().
 *
 ...
 */
EC_PUBLIC_API uint8_t *lib_ecrt_voe_handler_data(
        const ec_voe_handler_t *voe /**< VoE handler. */
        );
 ...
 */
EC_PUBLIC_API size_t lib_ecrt_voe_handler_data_size(
        const ec_voe_handler_t *voe /**< VoE handler. */
        );

/** Start a VoE write operation.
 *
 * After this function has been called, the lib_ecrt_voe_handler_execute() method
 ...
 *
 * \return 0 on success, otherwise negative error code.
 * \retval -ENOBUFS Reserved memory in lib_ecrt_slave_config_create_voe_handler
 *                  too small.
 */
EC_PUBLIC_API int lib_ecrt_voe_handler_write(
        ec_voe_handler_t *voe, /**< VoE handler. */
 ...
 * lib_ecrt_voe_handler_data_size(), while the VoE header of the received data
 * can be retrieved with lib_ecrt_voe_handler_received_header().
 *
 * This method is meant to be called in realtime context (after master
 * activation).
 *
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_voe_handler_read(
        ec_voe_handler_t *voe /**< VoE handler. */
        );
 ...
 * \return 0 on success, otherwise negative error code.
 */
EC_PUBLIC_API int lib_ecrt_voe_handler_read_nosync(
        ec_voe_handler_t *voe /**< VoE handler. */
        );
 ...
 *
 */
EC_PUBLIC_API ec_request_state_t lib_ecrt_voe_handler_execute(
        ec_voe_handler_t *voe /**< VoE handler. */
        );
 ...
 *   using the EC_READ_*() macros as usual. Example:
 *   \code
 *   uint16_t value = EC_READ_U16(lib_ecrt_reg_request_data(reg_request)));
 *   \endcode
 * - If a write operation shall be triggered, the data have to be written to
 *   the internal memory. Use the EC_WRITE_*() macros, if you are writing
 *   integer data. Be sure, that the data fit into the memory. The memory size
 *   is a parameter of lib_ecrt_slave_config_create_reg_request().
 *   \code
 *   EC_WRITE_U16(lib_ecrt_reg_request_data(reg_request), 0xFFFF);
 *   \endcode
 *
 * This method is meant to be called in realtime context (after master
 * activation), but can also be used to initialize data before.
 ...
 *
 */
EC_PUBLIC_API uint8_t *lib_ecrt_reg_request_data(
 ...
 *
 */
EC_PUBLIC_API ec_request_state_t lib_ecrt_reg_request_state(
        const ec_reg_request_t *req /**< Register request. */
    );

/** Schedule an register write operation.
 *
 * \attention This method may not be called while lib_ecrt_reg_request_state()
 * returns EC_REQUEST_BUSY.
 *
 ...
 *
 * \return 0 on success, otherwise negative error code.
 * \retval -ENOBUFS Reserved memory in lib_ecrt_slave_config_create_reg_request
 *              too small.
 */
EC_PUBLIC_API int lib_ecrt_reg_request_write(
 ...
 *              too small.
 */
EC_PUBLIC_API int lib_ecrt_reg_request_read(
        ec_reg_request_t *req, /**< Register request. */
        uint16_t address, /**< Register address. */
        size_t size /**< Size to write. */
        );
 ...
 * \return EtherCAT data value
 */
EC_PUBLIC_API float lib_ecrt_read_real(const void *data);
 ...
 * \return EtherCAT data value
 */
#define EC_READ_REAL(DATA) lib_ecrt_read_real(DATA)
 ...
 * \return EtherCAT data value
 */
EC_PUBLIC_API double lib_ecrt_read_lreal(const void *data);
 ...
 * \return EtherCAT data value
 */
#define EC_READ_LREAL(DATA) lib_ecrt_read_lreal(DATA)
 ...
 */
EC_PUBLIC_API void lib_ecrt_write_real(void *data, float value);
 ...
 * \param VAL new value
 */
#define EC_WRITE_REAL(DATA, VAL) lib_ecrt_write_real(DATA, VAL)
 ...
 * \param value new value
 */
EC_PUBLIC_API void lib_ecrt_write_lreal(void *data, double value);
 ...
 * \param VAL new value
 */
#define EC_WRITE_LREAL(DATA, VAL) lib_ecrt_write_lreal(DATA, VAL)
