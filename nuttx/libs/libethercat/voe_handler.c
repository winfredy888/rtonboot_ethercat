...
void lib_ec_voe_handler_clear(ec_voe_handler_t *voe)
{
    if (voe->data) {
        free(voe->data);
        voe->data = NULL;
...
int lib_ecrt_voe_handler_send_header(ec_voe_handler_t *voe, uint32_t vendor_id,
...
int lib_ecrt_voe_handler_received_header(const ec_voe_handler_t *voe,
...
uint8_t *lib_ecrt_voe_handler_data(const ec_voe_handler_t *voe)
{
    return voe->data;
}
...
size_t lib_ecrt_voe_handler_data_size(const ec_voe_handler_t *voe)
{
...
int lib_ecrt_voe_handler_read(ec_voe_handler_t *voe)
{
    ec_ioctl_voe_t data;
    int ret;
...
int lib_ecrt_voe_handler_read_nosync(ec_voe_handler_t *voe)
{
    ec_ioctl_voe_t data;
    int ret;
...
int lib_ecrt_voe_handler_write(ec_voe_handler_t *voe, size_t size)
{
    ec_ioctl_voe_t data;
    int ret;
...
ec_request_state_t lib_ecrt_voe_handler_execute(ec_voe_handler_t *voe)
{
    ec_ioctl_voe_t data;
    int ret;
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to execute VoE handler: %s\n",
    ...
    if (data.size) { // new data waiting to be copied
        if (voe->mem_size < data.size) {
            my_fprintf(stderr, "Received %zu bytes do not fit info VoE data"
        ...
        if (EC_IOCTL_IS_ERROR(ret)) {
            my_fprintf(stderr, "Failed to get VoE data: %s\n",
                    strerror(EC_IOCTL_ERRNO(ret)));
            return EC_REQUEST_ERROR;
        }
