...
void lib_ec_slave_config_clear(ec_slave_config_t *sc)
{
...
int lib_ecrt_slave_config_sync_manager(ec_slave_config_t *sc, uint8_t sync_index,
        ec_direction_t dir, ec_watchdog_mode_t watchdog_mode)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to config sync manager: %s\n",
...
int lib_ecrt_slave_config_watchdog(ec_slave_config_t *sc,
        uint16_t divider, uint16_t intervals)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to config watchdog: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
    return 0;
...
int lib_ecrt_slave_config_pdo_assign_add(ec_slave_config_t *sc,
        uint8_t sync_index, uint16_t pdo_index)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to add PDO: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_pdo_assign_clear(ec_slave_config_t *sc,
        uint8_t sync_index)
{
    ec_ioctl_config_pdo_t data;
    int ret;
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to clear PDOs: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
    return 0;
...
int lib_ecrt_slave_config_pdo_mapping_add(ec_slave_config_t *sc,
        ...
        uint8_t entry_bit_length)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to add PDO entry: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_pdo_mapping_clear(ec_slave_config_t *sc,
        uint16_t pdo_index)
{
    ec_ioctl_config_pdo_t data;
    int ret;
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to clear PDO entries: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
    return 0;
...
int lib_ecrt_slave_config_pdos(ec_slave_config_t *sc,
        unsigned int n_syncs, const ec_sync_info_t syncs[])
{
    ...
    for (i = 0; i < n_syncs; i++) {
        ...
        if (sync_info->index >= EC_MAX_SYNC_MANAGERS) {
            my_fprintf(stderr, "Invalid sync manager index %u!\n",
                    sync_info->index);
            return -ENOENT;
        }

        ret = lib_ecrt_slave_config_sync_manager(sc, sync_info->index,
                sync_info->dir, sync_info->watchdog_mode);
        if (ret)
            return ret;

        lib_ecrt_slave_config_pdo_assign_clear(sc, sync_info->index);
        ...
        if (sync_info->n_pdos && sync_info->pdos) {
            ...
            for (j = 0; j < sync_info->n_pdos; j++) {
                ...
                ret = lib_ecrt_slave_config_pdo_assign_add(
                        sc, sync_info->index, pdo_info->index);
                if (ret)
                    return ret;

                lib_ecrt_slave_config_pdo_mapping_clear(sc, pdo_info->index);

                if (pdo_info->n_entries && pdo_info->entries) {
                    for (k = 0; k < pdo_info->n_entries; k++) {
                        entry_info = &pdo_info->entries[k];

                        ret = lib_ecrt_slave_config_pdo_mapping_add(sc,
...
int lib_ecrt_slave_config_reg_pdo_entry(
        ...
        )
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to register PDO entry: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
    ...
    if (bit_position) {
        *bit_position = data.bit_position;
        ...
        if (data.bit_position) {
            my_fprintf(stderr, "PDO entry 0x%04X:%02X does not byte-align "
                    "in config %u:%u.\n", index, subindex,
...
int lib_ecrt_slave_config_reg_pdo_entry_pos(
        ec_slave_config_t *sc,
        uint8_t sync_index,
        ...
        )
{
    ...
    if (bit_position) {
        *bit_position = io.bit_position;
        ...
        if (io.bit_position) {
            my_fprintf(stderr, "PDO entry %u/%u/%u does not byte-align "
...
int lib_ecrt_slave_config_dc(ec_slave_config_t *sc, uint16_t assign_activate,
...
int lib_ecrt_slave_config_sdo(ec_slave_config_t *sc, uint16_t index,
        uint8_t subindex, const uint8_t *sdo_data, size_t size)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to configure SDO: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_complete_sdo(ec_slave_config_t *sc, uint16_t index,
...
int lib_ecrt_slave_config_sdo8(ec_slave_config_t *sc, uint16_t index,
        uint8_t subindex, uint8_t value)
{
    uint8_t data[1];

    EC_WRITE_U8(data, value);
    return lib_ecrt_slave_config_sdo(sc, index, subindex, data, 1);
}

/****************************************************************************/

int lib_ecrt_slave_config_sdo16(ec_slave_config_t *sc, uint16_t index,
        uint8_t subindex, uint16_t value)
{
    uint8_t data[2];

    EC_WRITE_U16(data, value);
    return lib_ecrt_slave_config_sdo(sc, index, subindex, data, 2);
}

/****************************************************************************/

int lib_ecrt_slave_config_sdo32(ec_slave_config_t *sc, uint16_t index,
        uint8_t subindex, uint32_t value)
{
    uint8_t data[4];

    EC_WRITE_U32(data, value);
    return lib_ecrt_slave_config_sdo(sc, index, subindex, data, 4);
}

/****************************************************************************/

int lib_ecrt_slave_config_emerg_size(ec_slave_config_t *sc, size_t elements)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to set emergency ring size: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_emerg_pop(ec_slave_config_t *sc, uint8_t *target)
{
    ec_ioctl_sc_emerg_t io;
    int ret;
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        if (EC_IOCTL_ERRNO(ret) != ENOENT) {
            my_fprintf(stderr, "Failed to get emergency message: %s\n",
                    strerror(EC_IOCTL_ERRNO(ret)));
        }
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_emerg_clear(ec_slave_config_t *sc)
{
    ec_ioctl_sc_emerg_t io;
    int ret;
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to clear emergency ring: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_emerg_overruns(const ec_slave_config_t *sc)
{
    ec_ioctl_sc_emerg_t io;
    int ret;
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to get emergency overruns: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
ec_sdo_request_t *lib_ecrt_slave_config_create_sdo_request(ec_slave_config_t *sc,
        uint16_t index, uint8_t subindex, size_t size)
{
    ...
    if (size) {
        ...
        if (!req->data) {
            my_fprintf(stderr, "Failed to allocate %zu bytes of SDO data"
                    " memory.\n", size);
            free(req);
            return 0;
        }
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to create SDO request: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        lib_ec_sdo_request_clear(req);
        free(req);
        return NULL;
    }
...
ec_soe_request_t *lib_ecrt_slave_config_create_soe_request(ec_slave_config_t *sc,
        uint8_t drive_no, uint16_t idn, size_t size)
{
    ...
    if (size) {
        req->data = malloc(size);
        if (!req->data) {
            my_fprintf(stderr, "Failed to allocate %zu bytes of SoE data"
                    " memory.\n", size);
            free(req);
            return 0;
        }
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to create SoE request: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        lib_ec_soe_request_clear(req);
        free(req);
        return NULL;
    }
...
ec_reg_request_t *lib_ecrt_slave_config_create_reg_request(ec_slave_config_t *sc,
        size_t size)
{
    ...
    if (size) {
        ...
        if (!reg->data) {
            my_fprintf(stderr, "Failed to allocate %zu bytes of register data"
                    " memory.\n", size);
            free(reg);
            return 0;
        }
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to create register request: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        lib_ec_reg_request_clear(reg);
        free(reg);
        return NULL;
    }
...
ec_voe_handler_t *lib_ecrt_slave_config_create_voe_handler(ec_slave_config_t *sc,
        size_t size)
{
    ec_ioctl_voe_t data;
    ec_voe_handler_t *voe;
    ...
    if (size) {
        ...
        if (!voe->data) {
            my_fprintf(stderr, "Failed to allocate %zu bytes of VoE data"
                    " memory.\n", size);
            free(voe);
            return 0;
        }
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to create VoE handler: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        lib_ec_voe_handler_clear(voe);
        free(voe);
        return NULL;
    }
...
int lib_ecrt_slave_config_state(const ec_slave_config_t *sc,
...
int lib_ecrt_slave_config_idn(ec_slave_config_t *sc, uint8_t drive_no,
        uint16_t idn, ec_al_state_t al_state, const uint8_t *data, size_t size)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to configure IDN: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_flag(ec_slave_config_t *sc, const char *key,
        int32_t value)
{
    ...
    if (!io.key) {
        my_fprintf(stderr, "Failed to allocate %zu bytes of flag key memory.\n",
                io.key_size);
        return -ENOMEM;
    }
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to configure feature flag: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_eoe_mac_address(ec_slave_config_t *sc,
        const unsigned char *mac_address)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to configure EoE MAC address: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_eoe_ip_address(ec_slave_config_t *sc,
        struct in_addr ip_address)
{
    ...
    ret = ioctl(sc->master->fd, EC_IOCTL_SC_EOE_IP_PARAM, &io);
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to configure EoE IP address: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_eoe_subnet_mask(ec_slave_config_t *sc,
        struct in_addr subnet_mask)
{
    ...
    ret = ioctl(sc->master->fd, EC_IOCTL_SC_EOE_IP_PARAM, &io);
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to configure EoE subnet mask: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_eoe_default_gateway(ec_slave_config_t *sc,
        struct in_addr gateway_address)
{
    ...
    ret = ioctl(sc->master->fd, EC_IOCTL_SC_EOE_IP_PARAM, &io);
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to configure EoE default gateway: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_eoe_dns_address(ec_slave_config_t *sc,
        struct in_addr dns_address)
{
    ...
    ret = ioctl(sc->master->fd, EC_IOCTL_SC_EOE_IP_PARAM, &io);
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to configure EoE DNS address: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_eoe_hostname(ec_slave_config_t *sc,
        const char *name)
{
    ec_ioctl_eoe_ip_t io = {};
    int ret;
    ...
    ret = ioctl(sc->master->fd, EC_IOCTL_SC_EOE_IP_PARAM, &io);
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to configure EoE hostname: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_slave_config_state_timeout(ec_slave_config_t *sc,
        ...
        unsigned int timeout_ms)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to configure AL state timeout: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
