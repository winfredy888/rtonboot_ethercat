...
void lib_ec_master_clear_config(ec_master_t *master)
{
...
extern int g_is_requested;

void lib_ec_master_clear(ec_master_t *master)
{
    lib_ec_master_clear_config(master);
...
ec_domain_t *lib_ecrt_master_create_domain(ec_master_t *master)
{
    ec_domain_t *domain;
    int index;
    ...
    if (EC_IOCTL_IS_ERROR(index)) {
        my_fprintf(stderr, "Failed to create domain: %s\n",
...
ec_slave_config_t *lib_ecrt_master_slave_config(ec_master_t *master,
        ...
        uint32_t product_code)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to create slave config: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        free(sc);
        return 0;
    }
...
int lib_ecrt_master_select_reference_clock(ec_master_t *master,
        ec_slave_config_t *sc)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to select reference clock: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master(ec_master_t *master, ec_master_info_t *master_info)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to get master info: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master_scan_progress(ec_master_t *master,
...
int lib_ecrt_master_get_slave(ec_master_t *master, uint16_t slave_position,
        ec_slave_info_t *slave_info)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to get slave info: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master_get_sync_manager(ec_master_t *master, uint16_t slave_position,
        uint8_t sync_index, ec_sync_info_t *sync)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to get sync manager information: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master_get_pdo(ec_master_t *master, uint16_t slave_position,
        uint8_t sync_index, uint16_t pos, ec_pdo_info_t *pdo)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to get pdo information: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master_get_pdo_entry(ec_master_t *master, uint16_t slave_position,
        ...
        ec_pdo_entry_info_t *entry)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to get pdo entry information: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master_sdo_download(ec_master_t *master,
        ...
        const uint8_t *data, size_t data_size, uint32_t *abort_code)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        ...
        my_fprintf(stderr, "Failed to execute SDO download: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master_sdo_download_complete(ec_master_t *master,
...
int lib_ecrt_master_sdo_upload(ec_master_t *master, uint16_t slave_position,
        ...
        size_t target_size, size_t *result_size, uint32_t *abort_code)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        ...
        my_fprintf(stderr, "Failed to execute SDO upload: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master_write_idn(ec_master_t *master, uint16_t slave_position,
        ...
        uint16_t *error_code)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        ...
        my_fprintf(stderr, "Failed to write IDN: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master_read_idn(ec_master_t *master, uint16_t slave_position,
        ...
        size_t *result_size, uint16_t *error_code)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        ...
        my_fprintf(stderr, "Failed to read IDN: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master_activate(ec_master_t *master)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to activate master: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
/*#ifdef USE_RTDM*/
        ...
        if (master->process_data == MAP_FAILED) {
            my_fprintf(stderr, "Failed to map process data: %s\n",
                    strerror(errno));
            master->process_data = NULL;
            master->process_data_size = 0;
            return -errno;
    ...
    while (domain) {
        ...
        if (EC_IOCTL_IS_ERROR(offset)) {
            my_fprintf(stderr, "Failed to get domain offset: %s\n",
...
int lib_ecrt_master_deactivate(ec_master_t *master)
{
    int ret;
...
int lib_ecrt_master_set_send_interval(ec_master_t *master,
...
int lib_ecrt_master_send(ec_master_t *master)
{
    int ret;
...
int lib_ecrt_master_receive(ec_master_t *master)
{
    int ret;
...
int lib_ecrt_master_state(const ec_master_t *master, ec_master_state_t *state)
{
    int ret;
...
int lib_ecrt_master_link_state(const ec_master_t *master, unsigned int dev_idx,
        ec_master_link_state_t *state)
{
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to get link state: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master_application_time(ec_master_t *master, uint64_t app_time)
{
    uint64_t time;
    int ret;
...
int lib_ecrt_master_sync_reference_clock(ec_master_t *master)
{
    int ret;
...
int lib_ecrt_master_sync_reference_clock_to(ec_master_t *master,
        uint64_t sync_time)
{
    uint64_t time;
    int ret;
...
int lib_ecrt_master_sync_slave_clocks(ec_master_t *master)
{
    int ret;
...
int lib_ecrt_master_reference_clock_time(const ec_master_t *master,
        uint32_t *time)
{
    int ret;
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        my_fprintf(stderr, "Failed to get reference clock time: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
...
int lib_ecrt_master_sync_monitor_queue(ec_master_t *master)
{
    int ret;
...
uint32_t lib_ecrt_master_sync_monitor_process(const ec_master_t *master)
{
    uint32_t time_diff;
    int ret;
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        ...
        my_fprintf(stderr, "Failed to process sync monitor datagram: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
    }
...
int lib_ecrt_master_reset(ec_master_t *master)
{
    int ret;
