...
extern int stmmac_send_packet(uint8_t * buffer, int txbuflength);

extern int stmmac_recv_packet(uint8_t * buffer, int len);
...
int ecx_setupnic(ecx_portt *port, const char *ifname, int secondary)
{
   ...
   nxmutex_init(&(port->getindex_mutex));
   nxmutex_init(&(port->tx_mutex));
   nxmutex_init(&(port->rx_mutex));
...
extern int g_soem_thread_keycount;

extern int g_soem_thread_keycount_saved;
...
int ecx_closenic(ecx_portt *port)
{
   ...
   g_soem_thread_keycount = g_soem_thread_keycount_saved;
...
uint8 ecx_getindex(ecx_portt *port)
{
   ...
   nxmutex_lock(&(port->getindex_mutex));
   ...
   nxmutex_unlock(&(port->getindex_mutex));
...
int ecx_outframe(ecx_portt *port, uint8 idx, int stacknumber)
{
   ...
   rval = stmmac_send_packet((*stack->txbuf)[idx], lp);
...
int ecx_outframe_red(ecx_portt *port, uint8 idx)
{
   ...
   if (port->redstate != ECT_RED_NONE)
   {
      ...
      stmmac_send_packet(&(port->txbuf2[0]), port->txbuflength2);
      
      nxmutex_unlock(&(port->tx_mutex));
   }
...
static int ecx_recvpkt(ecx_portt *port, int stacknumber)
{
   ...
   bytesrx = stmmac_recv_packet((*stack->tempbuf), lp);
