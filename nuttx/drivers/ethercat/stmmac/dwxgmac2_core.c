			       ...
			       struct net_driver_s *dev)
{
	...
	/*int mtu = nuttx_netdev_priv->mtu;*/
	u32 tx, rx;

	tx = getreg32((u64)(ioaddr + XGMAC_TX_CONFIG));
	rx = getreg32((u64)(ioaddr + XGMAC_RX_CONFIG));
	...
	putreg32(tx, (u64)(ioaddr + XGMAC_TX_CONFIG));
	putreg32(rx, (u64)(ioaddr + XGMAC_RX_CONFIG));
	putreg32(XGMAC_INT_DEFAULT_EN, (u64)(ioaddr + XGMAC_INT_EN));
}

static void dwxgmac2_set_mac(void __iomem *ioaddr, bool enable)
{
	u32 tx = getreg32((u64)(ioaddr + XGMAC_TX_CONFIG));
	u32 rx = getreg32((u64)(ioaddr + XGMAC_RX_CONFIG));
...
static int dwxgmac2_rx_ipc(struct mac_device_info *hw)
{
	...
	value = getreg32((u64)(ioaddr + XGMAC_RX_CONFIG));
	if (hw->rx_csum)
		value |= XGMAC_CONFIG_IPC;
	else
		value &= ~XGMAC_CONFIG_IPC;
	putreg32(value, (u64)(ioaddr + XGMAC_RX_CONFIG));

	return !!(getreg32((u64)(ioaddr + XGMAC_RX_CONFIG)) & XGMAC_CONFIG_IPC);
}
				     ...
				     u32 queue)
{
	...
	value = getreg32((u64)(ioaddr + XGMAC_RXQ_CTRL0)) & ~XGMAC_RXQEN(queue);
	if (mode == MTL_QUEUE_AVB)
		value |= 0x1 << XGMAC_RXQEN_SHIFT(queue);
	else if (mode == MTL_QUEUE_DCB)
		value |= 0x2 << XGMAC_RXQEN_SHIFT(queue);
	putreg32(value, (u64)(ioaddr + XGMAC_RXQ_CTRL0));
}
				   ...
				   u32 queue)
{
	...
	value = getreg32((u64)(ioaddr + reg));
	value &= ~XGMAC_PSRQ(queue);
	value |= (prio << XGMAC_PSRQ_SHIFT(queue)) & XGMAC_PSRQ(queue);

	putreg32(value, (u64)(ioaddr + reg));
}
					    ...
					    u32 rx_alg)
{
	...
	value = getreg32((u64)(ioaddr + XGMAC_MTL_OPMODE));
	value &= ~XGMAC_RAA;
	...
	putreg32(value, (u64)(ioaddr + XGMAC_MTL_OPMODE));
}
					    ...
					    u32 tx_alg)
{
	...
	for (i = 0; i < MTL_MAX_TX_QUEUES; i++) {
		value = getreg32((u64)(ioaddr + XGMAC_MTL_TCx_ETS_CONTROL(i)));
		value &= ~XGMAC_TSA;
		if (ets)
			value |= XGMAC_ETS;
		putreg32(value, (u64)(ioaddr + XGMAC_MTL_TCx_ETS_CONTROL(i)));
	}
}
					     ...
					     u32 weight, u32 queue)
{
	...
	putreg32(weight, (u64)(ioaddr + XGMAC_MTL_TCx_QUANTUM_WEIGHT(queue)));
}
				...
				u32 high_credit, u32 low_credit, u32 queue)
{
	...
	putreg32(send_slope, (u64)(ioaddr + XGMAC_MTL_TCx_SENDSLOPE(queue)));
	putreg32(idle_slope, (u64)(ioaddr + XGMAC_MTL_TCx_QUANTUM_WEIGHT(queue)));
	putreg32(high_credit, (u64)(ioaddr + XGMAC_MTL_TCx_HICREDIT(queue)));
	putreg32(low_credit, (u64)(ioaddr + XGMAC_MTL_TCx_LOCREDIT(queue)));

	value = getreg32((u64)(ioaddr + XGMAC_MTL_TCx_ETS_CONTROL(queue)));
	value &= ~XGMAC_TSA;
	value |= XGMAC_CC | XGMAC_CBS;
	putreg32(value, (u64)(ioaddr + XGMAC_MTL_TCx_ETS_CONTROL(queue)));
}
...
static void dwxgmac2_dump_regs(struct mac_device_info *hw, u32 *reg_space)
{
	...
	for (i = 0; i < XGMAC_MAC_REGSIZE; i++)
		reg_space[i] = getreg32((u64)(ioaddr + i * 4));
}
				    ...
				    struct stmmac_extra_stats *x)
{
	...
	en = getreg32((u64)(ioaddr + XGMAC_INT_EN));
	stat = getreg32((u64)(ioaddr + XGMAC_INT_STATUS));

	stat &= en;

	if (stat & XGMAC_PMTIS) {
		x->irq_receive_pmt_irq_n++;
		getreg32((u64)(ioaddr + XGMAC_PMT));
	}

	if (stat & XGMAC_LPIIS) {
		u32 lpi = getreg32((u64)(ioaddr + XGMAC_LPI_CTRL));
...
static int dwxgmac2_host_mtl_irq_status(struct mac_device_info *hw, u32 chan)
{
	...
	status = getreg32((u64)(ioaddr + XGMAC_MTL_INT_STATUS));
	if (status & BIT(chan)) {
		u32 chan_status = getreg32((u64)(ioaddr + XGMAC_MTL_QINT_STATUS(chan)));

		if (chan_status & XGMAC_RXOVFIS)
			ret |= CORE_IRQ_MTL_RX_OVERFLOW;

		putreg32(~0x0, (u64)(ioaddr + XGMAC_MTL_QINT_STATUS(chan)));
	}
			       ...
			       u32 tx_cnt)
{
	...
	if (fc & FLOW_RX)
		putreg32(XGMAC_RFE, (u64)(ioaddr + XGMAC_RX_FLOW_CTRL));
	if (fc & FLOW_TX) {
		for (i = 0; i < tx_cnt; i++) {
			u32 value = XGMAC_TFE;
			...
			putreg32(value, (u64)(ioaddr + XGMAC_Qx_TX_FLOW_CTRL(i)));
		}
	}
}
...
static void dwxgmac2_pmt(struct mac_device_info *hw, unsigned long mode)
{
	...
	if (val) {
		u32 cfg = getreg32((u64)(ioaddr + XGMAC_RX_CONFIG));
		cfg |= XGMAC_CONFIG_RE;
		putreg32(cfg,(u64)(ioaddr + XGMAC_RX_CONFIG));
	}

	putreg32(val, (u64)(ioaddr + XGMAC_PMT));
				   ...
				   unsigned char *addr, unsigned int reg_n)
{
	...
	putreg32(value | XGMAC_AE, (u64)(ioaddr + XGMAC_ADDRx_HIGH(reg_n)));

	value = (addr[3] << 24) | (addr[2] << 16) | (addr[1] << 8) | addr[0];
	putreg32(value, (u64)(ioaddr + XGMAC_ADDRx_LOW(reg_n)));
}
				   ...
				   unsigned char *addr, unsigned int reg_n)
{
	...
	hi_addr = getreg32((u64)(ioaddr + XGMAC_ADDRx_HIGH(reg_n)));
	lo_addr = getreg32((u64)(ioaddr + XGMAC_ADDRx_LOW(reg_n)));
				  ...
				  bool en_tx_lpi_clockgating)
{
	...
	value = getreg32((u64)(ioaddr + XGMAC_LPI_CTRL));

	value |= XGMAC_LPITXEN | XGMAC_LPITXA;
	if (en_tx_lpi_clockgating)
		value |= XGMAC_TXCGE;

	putreg32(value, (u64)(ioaddr + XGMAC_LPI_CTRL));
}
...
static void dwxgmac2_set_eee_timer(struct mac_device_info *hw, int ls, int tw)
{
	...
	putreg32(value, (u64)(ioaddr + XGMAC_LPI_TIMER_CTRL));
}
				...
				int mcbitslog2)
{
	...
	for (regs = 0; regs < numhashregs; regs++)
		putreg32(mcfilterbits[regs], (u64)(ioaddr + XGMAC_HASH_TABLE(regs)));
}
...
{
	struct stmmac_netdev_priv * nuttx_netdev_priv = netdev_rk_priv(dev);
	void __iomem *ioaddr = (void __iomem *)(nuttx_netdev_priv->base_addr);
	u32 value = getreg32((u64)(ioaddr + XGMAC_PACKET_FILTER));
	...
	if (dev->d_flags & IFF_PROMISC) {
		...
		for (i = 0; i < XGMAC_MAX_HASH_TABLE; i++)
			putreg32(~0x0, (u64)(ioaddr + XGMAC_HASH_TABLE(i)));
	}else if (!netdev_mc_empty(dev) && (dev->d_flags & IFF_MULTICAST)) {
	...
	/*if (netdev_uc_count(dev) > hw->unicast_filter_entries) {
		...
		for ( ; reg < XGMAC_ADDR_MAX; reg++) {
			putreg32(0, (u64)(ioaddr + XGMAC_ADDRx_HIGH(reg)));
			putreg32(0, (u64)(ioaddr + XGMAC_ADDRx_LOW(reg)));
		}
	}*/

	putreg32(value, (u64)(ioaddr + XGMAC_PACKET_FILTER));
}


static void dwxgmac2_set_mac_loopback(void __iomem *ioaddr, bool enable)
{
	u32 value = getreg32((u64)(ioaddr + XGMAC_RX_CONFIG));
				  ...
				  u32 val)
{
	...
	putreg32(val, (u64)(ioaddr + XGMAC_RSS_DATA));
	ctrl |= idx << XGMAC_RSSIA_SHIFT;
	ctrl |= is_key ? XGMAC_ADDRT : 0x0;
	ctrl |= XGMAC_OB;
	putreg32(ctrl, (u64)(ioaddr + XGMAC_RSS_ADDR));
				  ...
				  struct stmmac_rss *cfg, u32 num_rxq)
{
	...
	value = getreg32((u64)(ioaddr + XGMAC_RSS_CTRL));
	if (!cfg || !cfg->enable) {
		value &= ~XGMAC_RSSE;
		putreg32(value, (u64)(ioaddr + XGMAC_RSS_CTRL));
		return 0;
	}
				      ...
				      u16 perfect_match, bool is_double)
{
	...
	putreg32(hash, (u64)(ioaddr + XGMAC_VLAN_HASH_TABLE));
	...
	if (hash) {
		...
		value = getreg32((u64)(ioaddr + XGMAC_VLAN_TAG));
		...
		putreg32(value, (u64)(ioaddr + XGMAC_VLAN_TAG));
		...
		value &= ~XGMAC_VLAN_VID;
		putreg32(value | perfect_match, (u64)(ioaddr + XGMAC_VLAN_TAG));
	} else {
		u32 value = getreg32((u64)(ioaddr + XGMAC_PACKET_FILTER));
...
static void dwxgmac3_log_error(struct net_driver_s * ndev, u32 value, bool corr,
...
static void dwxgmac3_handle_mac_err(struct net_driver_s * ndev,
				    void __iomem *ioaddr, bool correctable,
				    struct stmmac_safety_stats *stats)
{
	u32 value;

	value = getreg32((u64)(ioaddr + XGMAC_MAC_DPP_FSM_INT_STATUS));
	putreg32(value, (u64)(ioaddr + XGMAC_MAC_DPP_FSM_INT_STATUS));
...
static void dwxgmac3_handle_mtl_err(struct net_driver_s * ndev,
				    void __iomem *ioaddr, bool correctable,
				    struct stmmac_safety_stats *stats)
{
	u32 value;

	value = getreg32((u64)(ioaddr + XGMAC_MTL_ECC_INT_STATUS));
	putreg32(value, (u64)(ioaddr + XGMAC_MTL_ECC_INT_STATUS));
...
static void dwxgmac3_handle_dma_err(struct net_driver_s * ndev,
				    void __iomem *ioaddr, bool correctable,
				    struct stmmac_safety_stats *stats)
{
	u32 value;

	value = getreg32((u64)(ioaddr + XGMAC_DMA_ECC_INT_STATUS));
	putreg32(value, (u64)(ioaddr + XGMAC_DMA_ECC_INT_STATUS));
...
static int dwxgmac3_safety_feat_config(void __iomem *ioaddr, unsigned int asp)
{
	...
	putreg32(0x0, (u64)(ioaddr + XGMAC_MTL_ECC_CONTROL));

	/* 2. Enable MTL Safety Interrupts */
	value = getreg32((u64)(ioaddr + XGMAC_MTL_ECC_INT_ENABLE));
	...
	putreg32(value, (u64)(ioaddr + XGMAC_MTL_ECC_INT_ENABLE));

	/* 3. Enable DMA Safety Interrupts */
	value = getreg32((u64)(ioaddr + XGMAC_DMA_ECC_INT_ENABLE));
	value |= XGMAC_DCEIE; /* Descriptor Cache Memory Correctable Error */
	value |= XGMAC_TCEIE; /* TSO Memory Correctable Error */
	putreg32(value, (u64)(ioaddr + XGMAC_DMA_ECC_INT_ENABLE));
	...
	value = getreg32((u64)(ioaddr + XGMAC_MAC_FSM_CONTROL));
	value |= XGMAC_PRTYEN; /* FSM Parity Feature */
	value |= XGMAC_TMOUTEN; /* FSM Timeout Feature */
	putreg32(value, (u64)(ioaddr + XGMAC_MAC_FSM_CONTROL));

	return 0;
}

static int dwxgmac3_safety_feat_irq_status(struct net_driver_s * ndev,
					   void __iomem *ioaddr,
					   unsigned int asp,
					   struct stmmac_safety_stats *stats)
{
	...
	mtl = getreg32((u64)(ioaddr + XGMAC_MTL_SAFETY_INT_STATUS));
	dma = getreg32((u64)(ioaddr + XGMAC_DMA_SAFETY_INT_STATUS));
...
{
	u32 val = getreg32((u64)(ioaddr + XGMAC_MTL_OPMODE));

	val &= ~XGMAC_FRPE;
	putreg32(val, (u64)(ioaddr + XGMAC_MTL_OPMODE));
...
static void dwxgmac3_rxp_enable(void __iomem *ioaddr)
{
	...
	val = getreg32((u64)(ioaddr + XGMAC_MTL_OPMODE));
	val |= XGMAC_FRPE;
	putreg32(val, (u64)(ioaddr + XGMAC_MTL_OPMODE));
}
					    ...
					    int pos)
{
	...
	for (i = 0; i < (sizeof(entry->val) / sizeof(u32)); i++) {
		...
		/* Write data */
		val = *((u32 *)&entry->val + i);
		putreg32(val, (u64)(ioaddr + XGMAC_MTL_RXP_IACC_DATA));

		/* Write pos */
		val = real_pos & XGMAC_ADDR;
		putreg32(val, (u64)(ioaddr + XGMAC_MTL_RXP_IACC_CTRL_ST));
			       ...
			       unsigned int count)
{
	...
	/* Force disable RX */
	old_val = getreg32((u64)(ioaddr + XGMAC_RX_CONFIG));
	val = old_val & ~XGMAC_CONFIG_RE;
	putreg32(val, (u64)(ioaddr + XGMAC_RX_CONFIG));
	...
	val |= nve & XGMAC_NVE;
	putreg32(val, (u64)(ioaddr + XGMAC_MTL_RXP_CONTROL_STATUS));
...
re_enable:
	/* Re-enable RX */
	putreg32(old_val, (u64)(ioaddr + XGMAC_RX_CONFIG));
	return ret;
}
...
static int dwxgmac2_get_mac_tx_timestamp(struct mac_device_info *hw, u64 *ts)
{
	...
	*ts = getreg32((u64)(ioaddr + XGMAC_TXTIMESTAMP_NSEC)) & XGMAC_TXTSSTSLO;
	*ts += getreg32((u64)(ioaddr + XGMAC_TXTIMESTAMP_SEC)) * 1000000000ULL;
	return 0;
}
...
{
	u32 tnsec = getreg32((u64)(ioaddr + XGMAC_PPSx_TARGET_TIME_NSEC(index)));
	u32 val = getreg32((u64)(ioaddr + XGMAC_PPS_CONTROL));
	u64 period;
	...
	if (!enable) {
		...
		putreg32(val, (u64)(ioaddr + XGMAC_PPS_CONTROL));
		return 0;
	}
	...
	putreg32(cfg->start.tv_sec, (u64)(ioaddr + XGMAC_PPSx_TARGET_TIME_SEC(index)));

	if (!(systime_flags & PTP_TCR_TSCTRLSSR))
		cfg->start.tv_nsec = (cfg->start.tv_nsec * 1000) / 465;
	putreg32(cfg->start.tv_nsec, (u64)(ioaddr + XGMAC_PPSx_TARGET_TIME_NSEC(index)));
	...
	putreg32(period - 1, (u64)(ioaddr + XGMAC_PPSx_INTERVAL(index)));

	period >>= 1;
	if (period <= 1)
		return -EINVAL;

	putreg32(period - 1, (u64)(ioaddr + XGMAC_PPSx_WIDTH(index)));
...
{
	u32 value = getreg32((u64)(ioaddr + XGMAC_TX_CONFIG));

	value &= ~XGMAC_CONFIG_SARC;
	value |= val << XGMAC_CONFIG_SARC_SHIFT;

	putreg32(value, (u64)(ioaddr + XGMAC_TX_CONFIG));
}
...
static void dwxgmac2_enable_vlan(struct mac_device_info *hw, u32 type)
{
	...
	value = getreg32((u64)(ioaddr + XGMAC_VLAN_INCL));
	value |= XGMAC_VLAN_VLTI;
	value |= XGMAC_VLAN_CSVL; /* Only use SVLAN */
	value &= ~XGMAC_VLAN_VLC;
	value |= (type << XGMAC_VLAN_VLC_SHIFT) & XGMAC_VLAN_VLC;
	putreg32(value, (u64)(ioaddr + XGMAC_VLAN_INCL));
}
				...
				u8 reg, u32 *data)
{
	...
	putreg32(value, (u64)(ioaddr + XGMAC_L3L4_ADDR_CTRL));

	ret = dwxgmac2_filter_wait(hw);
	if (ret)
		return ret;

	*data = getreg32((u64)(ioaddr + XGMAC_L3L4_DATA));
	return 0;
}
				 ...
				 u8 reg, u32 data)
{
	...
	putreg32(data, (u64)(ioaddr + XGMAC_L3L4_DATA));
				     ...
				     u32 match)
{
	...
	value = getreg32((u64)(ioaddr + XGMAC_PACKET_FILTER));
				     ...
				     u32 addr)
{
	...
	putreg32(addr, (u64)(ioaddr + XGMAC_ARP_ADDR));
...
static int dwxgmac3_est_write(void __iomem *ioaddr, u32 reg, u32 val, bool gcl)
{
	...
	putreg32(val, (u64)(ioaddr + XGMAC_MTL_EST_GCL_DATA));

	ctrl = (reg << XGMAC_ADDR_SHIFT);
	ctrl |= gcl ? 0 : XGMAC_GCRR;

	putreg32(ctrl, (u64)(ioaddr + XGMAC_MTL_EST_GCL_CONTROL));
				  ...
				  unsigned int ptp_rate)
{
	...
	ctrl = getreg32((u64)(ioaddr + XGMAC_MTL_EST_CONTROL));
	ctrl &= ~XGMAC_PTOV;
	...
	putreg32(ctrl, (u64)(ioaddr + XGMAC_MTL_EST_CONTROL));
				   ...
				   u32 num_rxq, bool enable)
{
	...
	if (!enable) {
		value = getreg32((u64)(ioaddr + XGMAC_FPE_CTRL_STS));

		value &= ~XGMAC_EFPE;

		putreg32(value, (u64)(ioaddr + XGMAC_FPE_CTRL_STS));
		return;
	}

	value = getreg32((u64)(ioaddr + XGMAC_RXQ_CTRL1));
	value &= ~XGMAC_RQ;
	value |= (num_rxq - 1) << XGMAC_RQ_SHIFT;
	putreg32(value, (u64)(ioaddr + XGMAC_RXQ_CTRL1));
				      ...
				      u32 queue)
{
	...
	value = getreg32((u64)(ioaddr + XLGMAC_RXQ_ENABLE_CTRL0)) & ~XGMAC_RXQEN(queue);
	if (mode == MTL_QUEUE_AVB)
		value |= 0x1 << XGMAC_RXQEN_SHIFT(queue);
	else if (mode == MTL_QUEUE_DCB)
		value |= 0x2 << XGMAC_RXQEN_SHIFT(queue);
	putreg32(value, (u64)(ioaddr + XLGMAC_RXQ_ENABLE_CTRL0));
}
