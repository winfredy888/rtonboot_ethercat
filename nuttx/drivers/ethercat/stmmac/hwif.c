...
{
	u32 reg = getreg32((u64)(priv->ioaddr + id_reg));

	if (!reg) {
		netdev_info(priv->dev, "Version ID not available\n");
		return 0x0;
	}
...
int stmmac_hwif_init(struct stmmac_priv *priv)
{
	...
	netdev_err(priv->dev, "Failed to find HW IF (id=0x%x, gmac=%d/%d)\n",
