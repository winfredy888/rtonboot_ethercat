...
static void dwmac5_log_error(struct net_driver_s * ndev, u32 value, bool corr,
...
static void dwmac5_handle_mac_err(struct net_driver_s * ndev,
		void __iomem *ioaddr, bool correctable,
		struct stmmac_safety_stats *stats)
{
	u32 value;

	value = getreg32((u64)(ioaddr + MAC_DPP_FSM_INT_STATUS));
	putreg32(value, (u64)(ioaddr + MAC_DPP_FSM_INT_STATUS));
...
static void dwmac5_handle_mtl_err(struct net_driver_s *ndev,
		void __iomem *ioaddr, bool correctable,
		struct stmmac_safety_stats *stats)
{
	u32 value;

	value = getreg32((u64)(ioaddr + MTL_ECC_INT_STATUS));
	putreg32(value, (u64)(ioaddr + MTL_ECC_INT_STATUS));
...
static void dwmac5_handle_dma_err(struct net_driver_s *ndev,
		void __iomem *ioaddr, bool correctable,
		struct stmmac_safety_stats *stats)
{
	u32 value;

	value = getreg32((u64)(ioaddr + DMA_ECC_INT_STATUS));
	putreg32(value, (u64)(ioaddr + DMA_ECC_INT_STATUS));
...
int dwmac5_safety_feat_config(void __iomem *ioaddr, unsigned int asp)
{
	...
	value = getreg32((u64)(ioaddr + MTL_ECC_CONTROL));
	value |= TSOEE; /* TSO ECC */
	value |= MRXPEE; /* MTL RX Parser ECC */
	value |= MESTEE; /* MTL EST ECC */
	value |= MRXEE; /* MTL RX FIFO ECC */
	value |= MTXEE; /* MTL TX FIFO ECC */
	putreg32(value, (u64)(ioaddr + MTL_ECC_CONTROL));

	/* 2. Enable MTL Safety Interrupts */
	value = getreg32((u64)(ioaddr + MTL_ECC_INT_ENABLE));
	...
	putreg32(value, (u64)(ioaddr + MTL_ECC_INT_ENABLE));

	/* 3. Enable DMA Safety Interrupts */
	value = getreg32((u64)(ioaddr + DMA_ECC_INT_ENABLE));
	value |= TCEIE; /* TSO Memory Correctable Error */
	putreg32(value, (u64)(ioaddr + DMA_ECC_INT_ENABLE));
	...
	value = getreg32((u64)(ioaddr + MAC_FSM_CONTROL));
	value |= PRTYEN; /* FSM Parity Feature */
	value |= TMOUTEN; /* FSM Timeout Feature */
	putreg32(value, (u64)(ioaddr + MAC_FSM_CONTROL));

	/* 4. Enable Data Parity Protection */
	value = getreg32((u64)(ioaddr + MTL_DPP_CONTROL));
	value |= EDPP;
	putreg32(value, (u64)(ioaddr + MTL_DPP_CONTROL));
...
int dwmac5_safety_feat_irq_status(struct net_driver_s *ndev,
		...
		struct stmmac_safety_stats *stats)
{
	...
	mtl = getreg32((u64)(ioaddr + MTL_SAFETY_INT_STATUS));
	dma = getreg32((u64)(ioaddr + DMA_SAFETY_INT_STATUS));
...
static int dwmac5_rxp_disable(void __iomem *ioaddr)
{
	...
	val = getreg32((u64)(ioaddr + MTL_OPERATION_MODE));
	val &= ~MTL_FRPE;
	putreg32(val, (u64)(ioaddr + MTL_OPERATION_MODE));
					  ...
					  int pos)
{
	...
	for (i = 0; i < (sizeof(entry->val) / sizeof(u32)); i++) {
		...
		/* Write data */
		val = *((u32 *)&entry->val + i);
		putreg32(val, (u64)(ioaddr + MTL_RXP_IACC_DATA));

		/* Write pos */
		val = real_pos & ADDR;
		putreg32(val, (u64)(ioaddr + MTL_RXP_IACC_CTRL_STATUS));
		      ...
		      unsigned int count)
{
	...
	/* Force disable RX */
	old_val = getreg32((u64)(ioaddr + GMAC_CONFIG));
	val = old_val & ~GMAC_CONFIG_RE;
	putreg32(val, (u64)(ioaddr + GMAC_CONFIG));
	...
	val = (nve << 16) & NPE;
	val |= nve & NVE;
	putreg32(val, (u64)(ioaddr + MTL_RXP_CONTROL_STATUS));
...
re_enable:
	/* Re-enable RX */
	putreg32(old_val, (u64)(ioaddr + GMAC_CONFIG));
	return ret;
}
...
{
	u32 tnsec = getreg32((u64)(ioaddr + MAC_PPSx_TARGET_TIME_NSEC(index)));
	u32 val = getreg32((u64)(ioaddr + MAC_PPS_CONTROL));
	u64 period;
	...
	if (!enable) {
		...
		val |= PPSEN0;
		putreg32(val, (u64)(ioaddr + MAC_PPS_CONTROL));
		return 0;
	}
	...
	putreg32(cfg->start.tv_sec, (u64)(ioaddr + MAC_PPSx_TARGET_TIME_SEC(index)));

	if (!(systime_flags & PTP_TCR_TSCTRLSSR))
		cfg->start.tv_nsec = (cfg->start.tv_nsec * 1000) / 465;
	putreg32(cfg->start.tv_nsec, (u64)(ioaddr + MAC_PPSx_TARGET_TIME_NSEC(index)));
	...
	putreg32(period - 1, (u64)(ioaddr + MAC_PPSx_INTERVAL(index)));

	period >>= 1;
	if (period <= 1)
		return -EINVAL;

	putreg32(period - 1, (u64)(ioaddr + MAC_PPSx_WIDTH(index)));
...
static int dwmac5_est_write(void __iomem *ioaddr, u32 reg, u32 val, bool gcl)
{
	...
	putreg32(val, (u64)(ioaddr + MTL_EST_GCL_DATA));

	ctrl = (reg << ADDR_SHIFT);
	ctrl |= gcl ? 0 : GCRR;

	putreg32(ctrl, (u64)(ioaddr + MTL_EST_GCL_CONTROL));
			 ...
			 unsigned int ptp_rate)
{
	...
	ctrl = getreg32((u64)(ioaddr + MTL_EST_CONTROL));
	ctrl &= ~PTOV;
	...
	putreg32(ctrl, (u64)(ioaddr + MTL_EST_CONTROL));
	return 0;
}
			  ...
			  bool enable)
{
	...
	if (!enable) {
		value = getreg32((u64)(ioaddr + MAC_FPE_CTRL_STS));

		value &= ~EFPE;

		putreg32(value, (u64)(ioaddr + MAC_FPE_CTRL_STS));
		return;
	}
