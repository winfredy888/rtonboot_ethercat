...
static void before_soemthing(uint8_t* buf)
{
...
void do_transform(uint32_t value)
{
	uint8_t key[16] = {
    ...
    before_soemthing(plaintext);
    
    memcpy(g_RTOSText, plaintext, sizeof(plaintext));		
}	

uint32_t do_soemthing(uint8_t * ptr_ciper_serial)
{
	uint8_t key[16] = {
        0x31, 0x39, 0x37, 0x31, 0x30, 0x33, 0x30, 0x32,
        0x31, 0x39, 0x37, 0x38, 0x31, 0x31, 0x32, 0x34
...
int main(int argc , char *argv[])
{
	...
	if(argc == 4)
	{
		if(strcmp(argv[1], "-i"))
		{
			printf("Wrong argv 1 exit\n");
			
			return -1;
		}
		
		temp = hex_to_int(argv[2]);
		if(!temp)
		{
			printf("Wrong serial exit\n");
			
			return -1;
		}	
		
		serial = (uint32_t)(temp);
	...
	if(is_input_serial)
	{
		fd_preact = open(argv[3], O_RDWR);
		
		if (fd_preact < 0) 
		{
			printf("cannot open pre act img file for rw %s\n", argv[3]);
	...
	read(fd_preact, &buf[0], len + 1);
	
	if(strcmp(&buf[0], PREACT_MAGIC))
	{
	...
	if(is_input_serial)
	{
		...
		write(fd_preact, g_RTOSText, 16);
		
		make_soemthing(serial);
		
		pos = 152;
		
		lseek(fd_preact, pos, SEEK_SET);
		
		write(fd_preact, g_soemThing, 32);
	}
	else
	{
		pos = 136;
		...
		serial = do_soemthing((uint8_t *)(&buf[0]));
		if(!serial)
		{
			printf("wrong serial\n");
