					 ...
					 struct rk_pcie *rk_pcie)
{
	...
	// 获取rp_rst_delay属性
    ret = of_property_read_u32(pdev->dev.of_node, "rp_rst_delay", &rk_pcie->rp_rst_delay);
    if (ret) {
        dev_err(&pdev->dev, "Failed to read rp_rst_delay, using default value\n");
		rk_pcie->rp_rst_delay=0;
		//rk_pcie->rp_rst_delay = 1500; // 如果设备树中没有rp_rst_delay属性，则使用默认值
    } else {
        dev_info(&pdev->dev, "rp_rst_delay value is %u\n", rk_pcie->rp_rst_delay);
    }
	/*(
	ret = of_property_read_u32(&pdev->dev, "rp_rst_delay", &rk_pcie->rp_rst_delay);
    if (ret) {
        dev_err(&pdev->dev, "Failed to read rp_rst_delay: %d\n", ret);
		rp_rst_delay = DEFAULT_RP_RST_DELAY;
    }else{
		dev_info(&pdev->dev, "rp_rst_delay value is %u\n", rp_rst_delay);
		rp_rst_delay = RP_RST_DELAY;
	}
	*/
