...
#ifdef CONFIG_RTONBOOT_ETHERCAT_SUPPORT	

int g_netport_idx = -1;

struct plat_stmmacenet_data * g_plat_dat = NULL;

struct stmmac_resources * g_stmmac_res = NULL;

unsigned char g_mac[7];

int stmmac_mdio_reset_before_nuttx(struct device *device)
{
	if (device->of_node) {
		struct gpio_desc *reset_gpio;
		u32 delays[3] = { 0, 0, 0 };

		reset_gpio = devm_gpiod_get_optional(device,
						     "snps,reset",
						     GPIOD_OUT_LOW);
		if (IS_ERR(reset_gpio))
			return PTR_ERR(reset_gpio);

		device_property_read_u32_array(device,
...
static int rk_gmac_probe(struct platform_device *pdev)
{
	...
	if (ret)
		goto err_remove_config_dt;
			...
			p_priv = (struct rk_priv_data *)(plat_dat->bsp_priv);
	
			if( (p_priv->bus_id) == 1)
			{
				g_netport_idx = 0;
			
				printk("Winfred Young got ethercat port need port init in nuttx port idx is %d\n", g_netport_idx);
			
				g_plat_dat = plat_dat;

				g_stmmac_res = &stmmac_res;
			
				rk_get_eth_addr(p_priv, &g_mac[0]);
			
				g_mac[6] = 0;
			
				rk_fix_speed(p_priv, 100);
			
				stmmac_mdio_reset_before_nuttx(&pdev->dev);
