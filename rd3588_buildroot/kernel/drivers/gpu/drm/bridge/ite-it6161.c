...
static void inline it6161_set_interrupts_active_level(struct it6161* it6161, enum it6161_active_level level)
{
					       ...
					       bool force)
{
	...
	it6161_set_interrupts_active_level(it6161, HIGH);
...
static bool getHDMITX_LinkStatus(struct it6161 *it6161)
{
...
*/
static void setHDMITX_LPCMAudio(struct it6161 *it6161, u8 AudioSrcNum, u8 AudSWL, u8 bAudInterface /*I2S/SPDIF/TDM*/)
{
...
static void setHDMITX_NLPCMAudio(struct it6161* it6161, u8 bAudInterface /*I2S/SPDIF/TDM*/) // no Source Num, no I2S.
{
    u8 AudioEnable, AudioFormat ;
    u8 i ;
...
static void setHDMITX_HBRAudio(struct it6161 *it6161, u8 bAudInterface /*I2S/SPDIF/TDM*/)
{
    // u8 rst;
    it6161_hdmi_tx_change_bank(it6161, 0);
...
static void setHDMITX_DSDAudio(struct it6161* it6161)
{
    // to be continue
    // u8 rst;
    // rst = it6161_hdmi_tx_read(it6161, REG_TX_SW_RST);
...
static void HDMITX_EnableAudioOutput(struct it6161 *it6161, u8 AudioType, u8 bAudInterface /*I2S/SPDIF/TDM*/,  u32 SampleFreq,  u8 ChNum, u8 *pIEC60958ChStat, u32 TMDSClock)
{
    ...
    #endif
        setHDMITX_NCTS(it6161, hdmiTxDev[0].bAudFs);
    ...
    case T_AUDIO_HBR:
        ...
        setHDMITX_HBRAudio(it6161, bAudInterface);

        break ;
    case T_AUDIO_DSD:
        DRM_INFO("T_AUDIO_DSD\n");
        setHDMITX_DSDAudio(it6161);
        break ;
    case T_AUDIO_NLPCM:
        DRM_INFO("T_AUDIO_NLPCM\n");
        pIEC60958ChStat[0] |= 1<<1 ;
        setHDMITX_ChStat(it6161, pIEC60958ChStat);
        setHDMITX_NLPCMAudio(it6161, bAudInterface);
        break ;
    case T_AUDIO_LPCM:
        ...
        setHDMITX_ChStat(it6161, pIEC60958ChStat);
        setHDMITX_LPCMAudio(it6161, (ChNum+1)/2, SUPPORT_AUDI_AudSWL, bAudInterface);
        // can add auto adjust
        break ;
    }
...
void hdmitx_AutoAdjustAudio()
{
    ...
    {
        hdmiTxDev[0].bAudFs=fs;
        setHDMITX_NCTS(it6161, hdmiTxDev[0].bAudFs); // set N, CTS by new generated clock.
        //CurrCTS=0;
        return;
    }
    return;
...
static void setHDMITX_NCTS(struct it6161* it6161, u8 Fs)
{
    u32 n;
	...
	if (reg08 & B_TX_INT_VIDSTABLE) {
		it6161_hdmi_tx_write(it6161, REG_TX_INT_STAT3, reg08);
			if (hdmi_tx_get_video_state(it6161)) {
				DRM_INFO("hdmi tx interrupt video stable link status:%d, rx reg0d:0x%02x start HDCP", getHDMITX_LinkStatus(it6161), it6161_mipi_rx_read(it6161, 0x0D));
...
static void mipirx_restart(struct work_struct *work)
{
	...
	it6161 = container_of(work, struct it6161, restart.work);

	it6161_debug("****it6161: %s\n", __func__);
	it6161_bridge_enable(&it6161->bridge);
}
			    ...
			    const struct i2c_device_id *id)
{
	...
	int err, intp_irq;
	struct drm_bridge* it6161_bridge; 

	struct it6161* it6161 = devm_kzalloc(dev, sizeof(*it6161), GFP_KERNEL);
	if (!it6161)
		return -ENOMEM;
