					       ...
					       u8 ntpal)
{
	...
	nvp6188_write_reg(client, 0x08 + ch, 0x00);
//modify display effect		
//	nvp6188_write_reg(client, 0x18 + ch, 0x10);
	nvp6188_write_reg(client, 0x0c + ch, 0x10);//brightness //00
	nvp6188_write_reg(client, 0x10 + ch, 0x88);//contrast //88
	nvp6188_write_reg(client, 0x14 + ch, 0x40);//h_sharpness v_sharpness //80 90	
	nvp6188_write_reg(client, 0x18 + ch, 0x00);//peak_mode fir_mode //0
	nvp6188_write_reg(client, 0x3c + ch, 0x80);//saturation //80
	nvp6188_write_reg(client, 0x40 + ch, 0x00);//HUE //0
	nvp6188_write_reg(client, 0x44 + ch, 0x00);//U_GAIN //0
	nvp6188_write_reg(client, 0x48 + ch, 0x00);//U_GAIN //0
//end	
