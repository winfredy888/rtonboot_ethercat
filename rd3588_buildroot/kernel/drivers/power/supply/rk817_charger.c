...
#define INPUT_1500MA	3000//1500¡¢2000¡¢2500¡¢3000
...
static int rk817_charge_get_charge_status(struct rk817_charger *charge)
{
	...
	is_rpdzkj_battery_exist = rk817_charge_field_read(charge, BAT_EXS);
	...
	DBG("rk817_charger : is_rpdzkj_battery_exist = %u\n", is_rpdzkj_battery_exist);
	if(is_rpdzkj_battery_exist == 0)
	{
		status = CC_OR_CV_CHRG ;
		DBG("rk817_charger : is_rpdzkj_battery_exist force CC or CV charge...\n");
	}
...
static int rk817_charge_parse_dt(struct rk817_charger *charge)
{
	...
	ret = of_property_read_u32(np, "rpdzkj_dc_vbus",
			&pdata->is_rpdzkj_dc_vbus);
	if (ret < 0 || !pdata->is_rpdzkj_dc_vbus)
		dev_err(dev, "not have to register rpdzkj_dc_vbus!\n");
...
static void rk817_charge_irq_delay_work(struct work_struct *work)
{
	...
	if (charge->plugin_trigger) {
		...
		//rpdzkj add
		if (charge->pdata->is_rpdzkj_dc_vbus) {
			DBG("is_rpdzkj_dc_vbus enable DC_to_vbus interface charging report \n");
			rk817_charge_set_chrg_param(charge, USB_TYPE_CDP_CHARGER);
		}
...
static int rk817_charge_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		...
		if (charge->pdata->is_rpdzkj_dc_vbus) { 
			DBG("is_rpdzkj_dc_vbus shield rk817_charge_usb_init error exit!!! \n");
		} else {
		return ret;
		}
	}
	...
	if (charge->pdata->is_rpdzkj_dc_vbus)
	{
		int status = rk817_charge_get_charge_status(charge);
		DBG("RPDZKJ status = %u\n", status );
		printk("RPDZKJ status = %u\n", status );
		if (status == 3 || status == 4)
		{

			charge->plugin_trigger = 1;	
			schedule_delayed_work(&charge->irq_work,0);
		}
	//   	rk817_charge_set_chrg_param(charge, USB_TYPE_CDP_CHARGER);	   
	}
