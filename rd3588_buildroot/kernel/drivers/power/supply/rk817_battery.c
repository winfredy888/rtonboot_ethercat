...
static int rk817_bat_get_charge_status(struct rk817_battery_device *battery)
{
	...
	is_rpdzkj_battery_exist = rk817_bat_field_read(battery, BAT_EXS);
	...
	DBG("rk817_battery : is_rpdzkj_battery_exist = %u\n", is_rpdzkj_battery_exist);
        if(is_rpdzkj_battery_exist == 0)
        {
                status = CC_OR_CV_CHRG ;
                DBG("rk817_battery : is_rpdzkj_battery_exist force CC or CV charge...\n");
        }
...
static int rk817_bat_parse_dt(struct rk817_battery_device *battery)
{
	...
	ret = of_property_read_u32(np, "dc_rpdzkj_psy",
			&battery->is_dc_rpdzkj_psy);
	if (ret < 0 || !battery->is_dc_rpdzkj_psy)
		dev_err(dev, "not have to register  dc_rpdzkj_psy!\n");
...
static void rk817_bat_lowpwr_check(struct rk817_battery_device *battery)
{
	...
	int rpdzkj_pwron_voltage = rk817_bat_get_pwron_voltage(battery) ;
	...
	if (battery->current_avg < 0 && battery->voltage_avg < pwr_off_thresd) {
		...
		if( rpdzkj_pwron_voltage > 2200          //Check whether the battery exists : yes   
				&& battery->is_dc_rpdzkj_psy
                                && battery->chip_id == RK817_ID ) {

			if ( battery->voltage_avg < pwr_off_thresd 
					|| battery->dsoc == 0 ) {
			
                        	if(battery->chrg_status == 3){  //Check whether it is in charging state : yes
                                	battery->dsoc = 1000;
                                	DBG("else rpdzkj dc in but battery level is 0, force battery level 1, soc=%d, current=%d\n",
                                    		battery->dsoc, battery->current_avg);
                        	} 
			}
                }

                if( rpdzkj_pwron_voltage < 2200    //Check whether the battery exists : not
                                && battery->is_dc_rpdzkj_psy 
                                && battery->chip_id == RK817_ID ) {

			if( (battery->voltage_avg > pwr_off_thresd && battery->dsoc <= 50)
			    || (battery->voltage_avg < 2800 && battery->chrg_status == 3) ){
                        	battery->dsoc = 50000;
                        	DBG("else rpdzkj dc in and not battery, <%s>,soc=%d, current=%d\n",
                            		__func__, battery->dsoc, battery->current_avg);
                        }
                }
	}

	DBG("<%s>. t=%lu, dsoc=%d, current=%d, voltage_avg =%d,fake_offline=%d\n",
	    __func__, base2sec(time), battery->dsoc,
	    battery->current_avg, battery->voltage_avg ,battery->fake_offline);
}
