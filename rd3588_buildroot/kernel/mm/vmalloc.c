...
struct vmap_area * rtonboot_alloc_vmap_area(uint64_t addr, unsigned long size,
				unsigned long align,
				unsigned long vstart, unsigned long vend,
				int node, gfp_t gfp_mask)
{
	...
	if (ret) {
		...
		printk("Winfred Young rtonboot_alloc_vmap_area kasan_populate_vmalloc fail\n");
...
struct vm_struct * rtonboot__get_vm_area_node(uint64_t addr, unsigned long size,
		unsigned long align, unsigned long flags, unsigned long start,
		unsigned long end, int node, gfp_t gfp_mask, const void *caller)
{
	struct vmap_area *va;
	...
	va = rtonboot_alloc_vmap_area(addr, size, align, start, end, node, gfp_mask);
	if (IS_ERR(va)) {
		kfree(area);
		return NULL;
	}
...
struct vm_struct * rtonboot_get_vm_area_caller(uint64_t addr, unsigned long size, unsigned long flags,
				const void *caller)
{
	return rtonboot__get_vm_area_node(addr, size, 1, flags, VMALLOC_START, VMALLOC_END,
				  NUMA_NO_NODE, GFP_KERNEL, caller);
}
