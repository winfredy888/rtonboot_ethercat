...
struct rtonboot_start_motorctl start_motorctl;

struct rtonboot_stop_motorctl stop_motorctl;

struct rtonboot_create_msg_desc motorctl_msgs[] = {
	{
    ...
    { 	
		.dst_msgid = MSGID_START_MOTORCTL,
		.max_content_size = 8,
		.msg_origin = MSG_ORIGIN_LINUX,
...
void prepare_callack_start_motorctl(uint32_t msgid, uint32_t * p_msglen, uint8_t ** pp_content, 
	uint32_t max_content_size, void * priv)
{
	(*p_msglen) = 4;
	
	start_motorctl.is_start = 1;
	start_motorctl.status = 0;
	
	*(pp_content) = (uint8_t *)(&start_motorctl);
}

void prepare_callack_stop_motorctl(uint32_t msgid, uint32_t * p_msglen, uint8_t ** pp_content, 
	uint32_t max_content_size, void * priv)
{
	(*p_msglen) = 4;
	
	stop_motorctl.is_stop = 1;
	stop_motorctl.status = 0;
	
	*(pp_content) = (uint8_t *)(&stop_motorctl);
}
...
void user_set_prepare_callback(uint32_t dst_msgid, rtsdk_prepare_send_callack_func_t * p_cb, void ** p_priv)
{
	switch (dst_msgid)
	{
		...
		case MSGID_START_MOTORCTL:
			...
			*p_cb = &prepare_callack_start_motorctl;
		...
		case MSGID_STOP_MOTORCTL:
			...
			*p_cb = &prepare_callack_stop_motorctl;
...
int main(int argc , char *argv[])
{
	...
	ret = RTSDKCreateMsgs(motorctl_msgs, 3);
	if(ret)
	{
		printf("Winfred Young create (motorctl_msgs msgs fail\n");
	...
	printf("EtherCAT begin send MSGID_START_MOTORCTL\n");
	
	ret = RTSDKSendMsg(MSGID_START_MOTORCTL);
	if(ret)
	{
		printf("Winfred Young send start motorctl msg fail\n");
    ...
    ret = RTSDKSendMsg(MSGID_STOP_MOTORCTL);
	if(ret)
	{
		printf("Winfred Young send stop motorctl msg fail\n");
