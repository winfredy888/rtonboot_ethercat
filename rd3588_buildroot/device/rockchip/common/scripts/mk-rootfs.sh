	...
	case "$ROOTFS" in
		...
		debian)
			if [ ! -f debian/linaro-rootfs.img ]; then
                                ...
                                ln -srf debian/linaro-rootfs.img $IMAGE_DIR/$ROOTFS_IMG
                        fi
                        ;;

		buildroot) build_buildroot "$IMAGE_DIR" ;;
		ubuntu)
			if [ ! -f ubuntu/ubuntu.img ]; then
				echo ""
				...
				ln -srf ubuntu/ubuntu.img $IMAGE_DIR/$ROOTFS_IMG
			fi
			;;
		...
		openkylin)
			if [ ! -f openkylin/openkylin.img ]; then
				...
				ln -srf openkylin/openkylin.img $IMAGE_DIR/$ROOTFS_IMG
			fi
			;;
