...
u64 get_tcr(int el, u64 *pips, u64 *pva_bits)
{
	...
	/* Find the largest address we need to support */
		...
		for (i = 0; winfred_mem_map[i].size || winfred_mem_map[i].attrs; i++)
			max_addr = max(max_addr, winfred_mem_map[i].virt + winfred_mem_map[i].size);
	...
	tcr_kernel = MMU_TCR_TG1(MMU_TG1(MMU_KERNEL_PAGE_SIZE_SHIFT));

	tcr_kernel |= MMU_TCR_SH1(MMU_SH_INNER_SHAREABLE);

	tcr_kernel |= MMU_TCR_ORGN1(MMU_RGN_WRITE_BACK_ALLOCATE);

	tcr_kernel |= MMU_TCR_IRGN1(MMU_RGN_WRITE_BACK_ALLOCATE);

	tcr_kernel |= MMU_TCR_T1SZ(64 - va_bits);

	//tcr_kernel |= MMU_TCR_EPD0;
...
static void add_map(struct mm_region *map, int is_kernel)
{
	u64 *pte;
	u64 virt = map->virt;
	u64 phys = map->phys;
	...
	if(is_kernel)
	{
		...
		/*attrs |= MMU_PTE_ATTR_UXN;

        attrs |= MMU_PTE_ATTR_PXN;

		attrs |= MMU_PTE_ATTR_AP_P_RW_U_NA;*/
	}
...
static int count_required_pts(u64 addr, int level, u64 maxaddr)
{
	...
	for (i = 0; winfred_mem_map[i].size || winfred_mem_map[i].attrs; i++) {
		...
		struct mm_region *map = &winfred_mem_map[i];
...
void setup_pgtables(void)
{
	...
	/* Now add all MMU table entries one after another to the table */
		...
		for (i = 0; winfred_mem_map[i].size || winfred_mem_map[i].attrs; i++)
			add_map(&winfred_mem_map[i], 0);

    #endif
}

void setup_pgtables_rtonboot(void)
{
	...
	if (!gd->arch.tlb_fillptr || !gd->arch.tlb_addr)
		...
		for (i = 0; rtonboot_mem_map[i].size || rtonboot_mem_map[i].attrs; i++)
			add_map(&rtonboot_mem_map[i], 1);
...
{
	u64 __maybe_unused asid_kernel;
	...
	u64 tlb_size = gd->arch.tlb_size;
		...
		rtos_printf("Winfred tlb_addr %llx tlb_size %llx \n", tlb_addr, tlb_size);
		...
		setup_pgtables_rtonboot();

		tlb_rtonboot = gd->arch.tlb_addr;
		...
		asid_kernel = MMU_ARM64_GLOBAL_ASID;

		asid_kernel <<= 48ULL;

		tlb_rtonboot |= asid_kernel;

	#endif
}

u64 MASTER_MAIR_VAL;
u64 MASTER_TCR_FLAGS;
u64 MASTER_TTBR0_EL1;
u64 MASTER_TTBR1_EL1;


extern void load_trampoline_vbar(void);
...
/*__weak*/ void mmu_setup(void)
{
	...
	el = current_el();
	set_ttbr_tcr_mair(el, tlb_normal, tlb_rtonboot, get_tcr(el, NULL, NULL),
			  MEMORY_ATTRIBUTES /*MMU_MAIR_VAL*/);
...
{
		uint64_t  __maybe_unused  temp;
	uint64_t * __maybe_unused  ptemp;
	...
	temp = MMU_ARM64_GLOBAL_ASID;
	
	#ifndef CONFIG_SPL_BUILD

	rtos_printf("Winfred asid kernel %llx\n", (long long unsigned int)temp);
 ...
 */
void /*__weak*/ winfred_enable_caches(void)
{
	icache_enable();
	dcache_enable();
}
