...
// 判断两个MAC地址的相似性（后5位相同）
int are_addresses_similar(const unsigned char *addr1, const unsigned char *addr2) {
    return memcmp(addr1 + 1, addr2 + 1, 5) == 0; // 比较后5个字节
}
	...
	for (i = 0; i < MAX_ETHERNET; i++) {
		if (ret <= 0 || !is_valid_ethaddr(&ethaddr[i * ARP_HLEN])) {
			if (!randomed) {
				...
				randomed = false;				//If you do not need a random address,randomed=true
			} else {
				if (i > 0) {
					...
					ethaddr[i * ARP_HLEN + ARP_HLEN - 1] += (i << 2); //If you do not need a random address, you can modify this line 
		...
		if (is_valid_ethaddr(&ethaddr[i * ARP_HLEN])) {
			...
			else if (are_addresses_similar(&ethaddr[0], &ethaddr[i*ARP_HLEN])) {
                     net_random_ethaddr(&ethaddr[i*ARP_HLEN]);
                     snprintf(buf, ARP_HLEN_ASCII + 1, "%pM", &ethaddr[i * ARP_HLEN]);
                     //printf("are_addresses_similar:%s\n",buf);
            	}
